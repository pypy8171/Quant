...
int main()
{
    ...
    // ── 한 봉 안 OHLC·누적 거래량 차, 다음 버킷 첫 틱이 앞 봉을 닫음, bar_index 0=최신 ──
    {
        ...
        CHECK(agg.snapshot(kSamsung)[1].close == 101.0);
        ...
        CHECK(agg.snapshot(kUnknown).empty());
    }
    ...
    // ── 장 밖·가격 0·다른 종목은 섞이지 않음, 종목별 독립 ────────────────────────
    {
        ...
        CHECK(agg.snapshot(kSamsung)[0].close == 100.0 && agg.snapshot(kHynix)[0].close == 50.0);
        agg.clear(kSamsung);
    ...
    // ── acml 없는 틱(REST 대체·선물)은 qty 합산으로 뒷걸음 ────────────────────────
    {
        ...
        CHECK(agg.snapshot(kSamsung)[0].volume == 7);
        // 중간에 acml이 붙어도 기준값이 없으면 계속 합산한다.
        CHECK(agg.on_tick(tick("100200", 12.0, 5, 999)));
        CHECK(agg.snapshot(kSamsung)[0].volume == 12);
    }
    ...
    // ── 시드: 빈 상태에 REST 봉 넣기, 닫힌 봉은 REST가 이김, 진행 봉은 합침 ──────────
    {
        ...
        CHECK(agg.snapshot(kSamsung)[0].open == 200.0 && agg.snapshot(kSamsung)[0].volume == 20);
        ...
        CHECK(agg.snapshot(kSamsung)[0].volume == 485 && agg.snapshot(kSamsung)[0].high == 207.0);
        ...
        CHECK(agg.seed(kSamsung, again) == 0);
        CHECK(agg.snapshot(kSamsung)[1].close == 199.0 && agg.snapshot(kSamsung)[1].volume == 310);
        ...
        CHECK(agg.seed(kSamsung, ahead) == 1);
    ...
    // ── keep 상한: 오래된 봉부터 버림, 슬롯 순서 유지 ────────────────────────────
    {
        ...
        CHECK(agg.closed_count(kSamsung) == 3 && agg.snapshot(kSamsung)[3].close == 102.0);
    }
    ...
    // ── 시계 확정(close_stale): 다음 틱 없이도 분이 지나면 닫힌다 ──────────────────────────────── [D-074]
    {
        ...
        CHECK(agg.close_stale(kSamsung, utc_of("153030")) == 0); // 같은 분 — 아직
        CHECK(agg.close_stale(kUnknown, utc_of("153100")) == 0);
        CHECK(agg.close_stale(kSamsung, utc_of("153100")) == 1); // 15:31 시계가 15:30 봉을 닫는다(장 필터 무관)
        CHECK(closed.size() == 2 && closed[1].close == 101.0 && closed[1].volume == 2);
        CHECK(!agg.current_slot(kSamsung).valid() && agg.closed_count(kSamsung) == 2);
        ...
        CHECK(agg.current_slot(kNaver).bucket == 10 * 60 + 1 && agg.closed_count(kNaver) == 0);
        CHECK(agg.close_stale(kNaver, utc_of("100200")) == 1);
