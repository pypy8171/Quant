...
//   처리 파이프라인의 E2E 지연 분포(p50/p99/p999)와 최대 지속가능 throughput
//   ("용량 천장")을 실측한다.
//
//   ⚠ 정직 경계 — 본 하네스는 *내부 처리단*만 측정한다. 실제 KIS REST/WS 네트워크
//     지연은 포함하지 않는다. 프로덕션 end-to-end 지연은 무료 API 폴링 주기(초 단위)에
//     지배당하며, 그것이 진짜 병목이다. 여기서 증명하려는 건 "처리단은 전종목 규모에서도
//     µs로 여유가 있다 → 병목은 링버퍼가 아니라 피드다"라는 명제다.
//
...
//   - 종료 후 드레인 구간 항목은 percentile 오염원 → 카운트만, 샘플 제외.
