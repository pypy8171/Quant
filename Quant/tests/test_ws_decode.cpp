...
static void test_orderbook()
{
    ...
    OrderBook ob;
    assert(kis_ws::decode_orderbook(f, ob) == Decode::kOk);
    ...
    f.pop_back();
    OrderBook untouched;
    assert(kis_ws::decode_orderbook(f, untouched) == Decode::kShort);
    ...
    f[5] = "";
    OrderBook partial;
    assert(kis_ws::decode_orderbook(f, partial) == Decode::kBadNumber);
...
static void test_fut_orderbook()
{
    ...
    OrderBook ob;
    assert(kis_ws::decode_fut_orderbook(f, ob) == Decode::kOk);
    ...
    OrderBook ob2;
    assert(kis_ws::decode_fut_orderbook(f, ob2) == Decode::kOk);

    f.resize(31);
    assert(kis_ws::decode_fut_orderbook(f, ob2) == Decode::kShort);
}
...
static void test_kr_trade()
{
    ...
    f[21] = "5";
    TradeData td;
    assert(kis_ws::decode_kr_trade(f, td) == Decode::kOk);
...
static void test_us_trade()
{
    ...
    f[8] = "120";
    TradeData td;
    assert(kis_ws::decode_us_trade(f, td) == Decode::kOk);
...
static void test_fut_trade()
{
    ...
    assert(kis_ws::decode_fut_trade(f, td) == Decode::kOk);
    assert(td.price == 412.35 && td.quantity == 3 && td.direction == 0);
    assert(td.market == Market::KR);

    f.resize(18);
    assert(kis_ws::decode_fut_trade(f, td) == Decode::kShort);
}
...
{
    FillNotification fn;
    assert(kis_ws::decode_fill(fill_record("02", "2"), fn) == Decode::kOk);
    ...
    assert(kis_ws::decode_fill(fill_record("01", "2"), sell) == Decode::kOk);
    assert(sell.side == OrderSide::SELL);

    // 접수 통보(1)는 체결이 아니다.
    FillNotification skip;
    assert(kis_ws::decode_fill(fill_record("02", "1"), skip) == Decode::kSkip);
    assert(skip.odno.empty());

    // 매매구분이 01/02 밖이면 원장에 못 넣는다 — 채우지 않고 돌려보낸다.
    FillNotification bad_side;
    assert(kis_ws::decode_fill(fill_record("03", "2"), bad_side) == Decode::kBadSide);
    ...
    assert(kis_ws::decode_fill(f, bad_num) == Decode::kBadNumber);

    // 1~2필드 제어 메시지와 13필드 모두 kShort.
    FillNotification s;
    assert(kis_ws::decode_fill({"0", "1"}, s) == Decode::kShort);
    assert(kis_ws::decode_fill(blank(13), s) == Decode::kShort);
}
