...
//   코스콤이 수백~수천 종목의 호가/체결을 서버로 밀어넣으면, 서버는 그 데이터를
//   받아 시스템 매매 파이프라인(전략→주문)으로 흘린다. 이 하네스는 그 "수신 + 처리"
//   경로를 **실제 TCP 소켓(loopback)**으로 통과시켜, wire 송신 시각 → 주문 결정
//   시각까지의 end-to-end 지연을 네트워크 구간과 처리 구간으로 분해 측정한다.
//
//   bench_market_firehose(자매 하네스)는 소켓 없이 프로세스 내부 처리단만 격리
//   측정했다("순수 처리 비용"). 이 하네스는 그 위에 **커널 TCP 스택 왕복(recv)**을
//   더해, 실제 피드 수신 서버가 겪는 경로를 측정한다.
//
//   ⚠ 정직 경계 — loopback TCP는 실제 코스콤↔증권사 WAN/전용선이 아니다. 커널
//     네트워크 스택(send→loopback→recv)·직렬화·프레이밍·백프레셔는 실제로 통과하지만,
//     물리 회선 지연(수십~수백µs)은 이 머신에 없다. 즉 "수신 서버 내부에서 데이터가
//     소켓에 도착한 뒤 주문 결정까지" + "동일 머신 TCP 스택 비용"을 측정한다.
//     실제 코스콤 실시간 데이터는 소량(~20종목)을 KIS WS로 병행 실증한다(별도).
//
// 토폴로지:
