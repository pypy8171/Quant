...
// 
// DEBUG
//[1] std::map<string, Data>        : 73580 milliseconds
//[2] std::unordered_map<string, D> : 9521 milliseconds
//[3] Flat Array (Security ID)      : 444 milliseconds
// 
// RELEASE
// [1] std::map<string, Data>           : 8104 milliseconds (개당 약 8104 nanoseconds)
// [2] std::unordered_map<string, D>    : 1290 milliseconds(개당 약 1290 nanoseconds)
// [3] Flat Array(Security ID)          : 0 milliseconds(개당 약 0 nanoseconds)
//================================================================
...
int main()
{
    ...
    for (size_t iteration_index = 0; iteration_index < ITERATIONS; ++iteration_index)
    {
        ...
        input_stream_string.push_back(tickers[index]);
    ...
    // -------------------------------------------------------------------------
    {
        ...
        for (size_t iteration_index = 0; iteration_index < ITERATIONS; ++iteration_index)
        {
            ...
            const auto& data = ticker_map[input_stream_string[iteration_index]];
            checksum += data.price;
        }
    ...
    // -------------------------------------------------------------------------
    {
        ...
        for (size_t iteration_index = 0; iteration_index < ITERATIONS; ++iteration_index)
        {
            ...
            const auto& data = ticker_unmap[input_stream_string[iteration_index]];
            checksum += data.price;
        }
    ...
    {
        // 2700개 종목에 대한 연속된 고정 메모리 물리 배열 선언
        std::vector<MarketData> vector;
        vector.resize(TICKER_COUNT);
        std::fill(vector.begin(), vector.end(), MarketData{1000, 100});
        ...
        {
            // Hot-Path: 문자열 배제, 정수 ID로 다이렉트 주소 이동 (O(1))
            const auto& data = vector[input_stream_id[iteration_index]];
            checksum += data.price;
        }
