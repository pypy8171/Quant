...
void DeviationScaleStrategy::on_trade_batch(const TradeData& trade, std::vector<OrderSignal>& out)
{
    ...
    const std::string buy_context = entry_kind + " 이격=" + format_one_decimal(deviation20_percent) +
                                    "% 일봉SMA20=" + format_one_decimal(d_s20) +
                                    " 현재가=" + format_one_decimal(current_price) + entry_context;
    ...
    LOG_INFO("[" + id() + "] 분할 매수 재구성 sma=" + format_one_decimal(simple_moving_average) +
             (warming ? "(일봉)" : "") + " src=" + (local_bars ? "ws" : "rest") +
