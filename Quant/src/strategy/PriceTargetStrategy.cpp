...
const std::string& PriceTargetStrategy::id() const
{
...
std::string PriceTargetStrategy::describe() const
{
    ...
    for (const auto& target : targets_)
    {
        text += " | " + target.ticker + " B=" + std::to_string(static_cast<int>(target.buy_price)) +
                " S=" + std::to_string(static_cast<int>(target.sell_price)) + " qty=" + std::to_string(target.quantity);
    }

    for (const auto& low : limit_orders_)
    {
        text += " | " + low.ticker + " LMT@" + std::to_string(static_cast<int>(low.price)) + " x" +
                std::to_string(low.quantity);
    }

    return text;
}

std::vector<WatchSpec> PriceTargetStrategy::get_watch_specifications() const
{
...
void PriceTargetStrategy::on_start()
{
    ...
    for (const auto& target : targets_)
    {
        LOG_INFO("  가격목표: " + target.ticker + " BUY≤" + std::to_string(static_cast<int>(target.buy_price)) +
                 " SELL≥" + std::to_string(static_cast<int>(target.sell_price)) +
                 " qty=" + std::to_string(target.quantity) + " cooldown=" + std::to_string(target.cooldown_sec) + "s");
    }

    for (const auto& low : limit_orders_)
    {
        LOG_INFO("  예약지정가: " + low.ticker + (low.side == OrderSide::BUY ? " BUY" : " SELL") + " @" +
                 std::to_string(static_cast<int>(low.price)) + " x" + std::to_string(low.quantity));
    }
}

std::optional<OrderSignal> PriceTargetStrategy::on_order_book(const OrderBook& order_book)
{
    // 지정가 예약 주문 먼저
...
std::optional<OrderSignal> PriceTargetStrategy::on_trade(const TradeData& trade)
{
...
void PriceTargetStrategy::on_stop()
{
    LOG_INFO("[PriceTarget] 종료");
}

std::optional<OrderSignal> PriceTargetStrategy::check_limit_order(symbol::SymbolId symbol_id, std::string_view ticker)
{
    for (auto& low : limit_orders_)
    {
        ...
        LOG_INFO("[PriceTarget] 예약지정가 제출: " + low.ticker + (low.side == OrderSide::BUY ? " BUY" : " SELL") +
                 " @" + std::to_string(static_cast<int>(low.price)) + " x" + std::to_string(low.quantity));
        return signal;
    }
...
std::optional<OrderSignal> PriceTargetStrategy::check_price_target(symbol::SymbolId symbol_id, std::string_view ticker,
                                                                   double price, int32_t hhmmss)
{
    if (price <= 0)
    {
        return std::nullopt;
    ...
    for (size_t target_index = 0; target_index < targets_.size(); ++target_index)
    {
        ...
        {
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - last_buy_[target_index]).count();

            if (elapsed >= target.cooldown_sec)
            {
                last_buy_[target_index] = now;
                LOG_INFO("[PriceTarget] BUY 조건 충족: " + std::string(ticker) + " @" +
                         std::to_string(static_cast<int>(price)) + " (목표≤" +
                         std::to_string(static_cast<int>(target.buy_price)) + ")");
        ...
        {
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - last_sell_[target_index]).count();
            ...
            if (elapsed >= target.cooldown_sec)
            {
                ...
                LOG_INFO("[PriceTarget] SELL 조건 충족: " + std::string(ticker) + " @" +
                         std::to_string(static_cast<int>(price)) + " (목표≥" +
                         std::to_string(static_cast<int>(target.sell_price)) + ")");
...
OrderSignal PriceTargetStrategy::make_signal(const PriceTarget& price_target, OrderSide side, double trigger_price)
{
    OrderSignal signal;
