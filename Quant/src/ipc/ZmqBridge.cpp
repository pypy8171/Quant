...
ZmqBridge::ZmqBridge(int pub_port, int rep_port)
    : pub_port_(pub_port), rep_port_(rep_port)
{}

ZmqBridge::~ZmqBridge() { stop(); }
    ...
    while (running_.load()) {
        ...
        try {
            ...
            if (items[0].revents & ZMQ_POLLIN) {
                ...
                if (cmd_handler_) {
                    try { reply_str = cmd_handler_(cmd); } catch (...) {}
                }
            ...
            if (e.num() != ETERM) LOG_WARN(std::string("[ZMQ] poll 오류: ") + e.what());
        }
    }
