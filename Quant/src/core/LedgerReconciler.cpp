...
{
    auto& position_ledger = gate_.ledger();

    // 대조 행은 덮어쓰기·정리 "전" 원장 값으로 남긴다 — 덮어쓴 뒤에 재면 항상 일치로 나온다. [why D-038]
    std::vector<reconcile::Held> ledger_before;

    for (const auto& held_position : position_ledger.snapshot_positions())
    {
    ...
    if (resync_positions)
    {
        position_ledger.reset_reserved();
    }
    ...
    for (const Holding& holding : balance.holdings)
    {
        ...
        if (resync_positions)
        {
            position_ledger.seed_position(std::string(), code, quantity, average_value);
        }

        // 매도가능수량은 재동기 모드와 무관하게 매번 맞춘다(기동 시드 0 고착 해소).
        if (holding.sellable_quantity)
        {
            position_ledger.refresh_sellable(std::string(), code, *holding.sellable_quantity);
        }
        ...
        if (!resync_positions)
        {
            const int absorbed = position_ledger.absorb_missed_sell(std::string(), code, quantity);
    ...
    if (!gone.empty())
    {
        ...
        for (const symbol::SymbolId gone_symbol : gone)
        {
            ...
            list += position_ledger.symbols().name(gone_symbol).view();
        }
...
void LedgerReconciler::reconcile(bool resync_positions, std::time_t now_utc)
{
    ...
    try
    {
        ...
        else
        {
            ...
            {
                position_ledger.set_available_cash(*balance->available_cash);
            }
            ...
            if (balance->total_evaluation_amount)
            {
                ...
                position_ledger.set_daily_pnl(delta); // 손실컷용(세션 기준점) — 리스크게이트 동작 유지
                position_ledger.set_equity(total_evaluation); // 총노출 게이트(§3d) 분모 — 총평가금 스냅샷 갱신
