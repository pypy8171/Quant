...
std::optional<OrderPacer::Pending> OrderPacer::take_due_retry(Clock::time_point now)
{
    ...
    {
        Retry retry = std::move(retry_q_.front());
        retry_q_.pop_front();
                             ...
                             Clock::time_point now)
{
    ...
    retry_q_.push_back({pending.signal, pending.attempts + 1, now + plan.delay});
    ...
    {
        const std::string act = pending.signal.action == OrderAction::CANCEL    ? "CANCEL"
                                : pending.signal.action == OrderAction::REPLACE ? "REPLACE"
                                                                       : "NEW";
        LOG_WARN("[OrderThread] 유량한도 거부 → 재시도 예약 " + pending.signal.ticker + " " + act + tries);
    }
    else
    {
