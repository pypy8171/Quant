...
// 스레드: 읽기(mask)는 수신·데이터 스레드가 틱마다 락 없이(원자 load). 쓰기(rebuild)는 전략 목록이 바뀔 때
//  strategy_.mutex 하에서만. 종목 하나의 마스크는 옛 값→새 값으로 한 번에 바뀌므로 읽는 쪽이 반쯤 만든 표를 볼 일은 없다.
#pragma once
...
public:
    ...
    explicit RouteTable(size_t symbol_capacity = 8192)
        : capacity_(symbol_capacity), masks_(std::make_unique<std::atomic<ShardMask>[]>(symbol_capacity))
    {
        for (size_t index = 0; index < capacity_; ++index)
        {
            masks_[index].store(0, std::memory_order_relaxed);
        }
    }
    ...
    {
        capacity_ = symbol_capacity;
        masks_    = std::make_unique<std::atomic<ShardMask>[]>(symbol_capacity);
        ...
        all_.store(0, std::memory_order_relaxed);
    }
    ...
    {
        return Draft(capacity_);
    }

    // 종목마다 원자 store 하나. 읽는 쪽은 종목 단위로 옛 표 아니면 새 표를 본다.
    void commit(const Draft& draft)
    {
        all_.store(draft.all, std::memory_order_release);

        for (size_t index = 0; index < capacity_ && index < draft.masks.size(); ++index)
        {
            masks_[index].store(draft.masks[index], std::memory_order_release);
        }
    }
    ...
    {
        const ShardMask all = all_.load(std::memory_order_acquire);
        return id != symbol::kNone && id < capacity_ ? masks_[id].load(std::memory_order_acquire) | all : all;
    }
...
private:
    size_t                                     capacity_;
    std::unique_ptr<std::atomic<ShardMask>[]>  masks_;
    std::atomic<ShardMask>                     all_{0};
};
