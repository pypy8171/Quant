...
//  체결통보 콜백은 on_tick에서만 부른다 — pipeline_.fill_queue의 생산자를 하나로 두기 위해. 부르는 스레드는
//  시세가 오는 판에서는 피드 스레드, 갈라 띄운 주문 프로세스에서는 주문 스레드다(그쪽은 시세가 없어 주문
//  스레드가 접수 뒤에 틱을 지어 먹인다 — Engine::feed_paper_fill_tick). 어느 쪽이든 생산자는 하나다.
