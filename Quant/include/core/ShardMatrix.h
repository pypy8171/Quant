...
//  생산자 n(소켓 하나를 읽는 수신 스레드)이 ShardRoutes 마스크가 가리키는 샤드 m마다 셀 [n][m]에 push_to하고,
//  소비자 m(전략 샤드 스레드)이 자기 열 [*][m]을 라운드로빈으로 pop한다. 종목 해시로 고른 열(consumer_of)은
//  그 종목을 보는 샤드가 없을 때의 대체 열이다(해시로 고르는 push()는 지금 부르는 곳이 없다). 셀마다 생산자·소비자가
//  하나라 락 없는 RingBuffer가 그대로 쓰인다(원칙 5 — MPSC 하나보다 N×M SPSC를 먼저). 종목은 소켓 하나에만
//  있으므로(FeedMux) 한 종목의 틱은 그 종목을 보는 샤드마다 셀 하나씩을 지나고, 샤드마다 순서가 지켜진다
//  (원칙 2, D-110). 채널·종목 사이 순서는 지키지 않는다.
