...
namespace jsonx
{
// [inv] 반환 참조는 j의 수명 안, j에 삽입이 없는 구간에서만 유효하다. 빈 노드는 정적이라 언제나 산다.
inline const nlohmann::json& array_or_empty(const nlohmann::json& j, const char* key)
{
    static const nlohmann::json kEmpty = nlohmann::json::array();
    const auto                  it     = j.is_object() ? j.find(key) : j.end();
    return (it != j.end() && it->is_array()) ? *it : kEmpty;
}

// [inv] array_or_empty와 같다.
inline const nlohmann::json& object_or_empty(const nlohmann::json& j, const char* key)
{
