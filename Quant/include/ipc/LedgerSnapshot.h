...
//  문자열도 포인터도 담지 않는다. 단계 4에서 이 구조체가 공유메모리에 그대로 얹히기 때문이다 — 주소는 프로세스마다
//  다르므로 포인터를 담으면 건너간 쪽에서 엉뚱한 곳을 가리킨다. 종목은 전부 정수 id 배열 인덱스다. [why D-114]
//
...
//  언저리의 수십 줄이다. 이 폭을 넘겨 쓰면 읽는 쪽이 굶는다 — 쓰는 창이 넓으면 읽는 쪽이 판 번호가
