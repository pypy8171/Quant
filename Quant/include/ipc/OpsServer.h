...
public:
    ...
    // 배포 교체가 부르는 곱게 내리기. 킬과 달리 되돌릴 것이 없다 — 감시견이 새 exe로 다시 띄운다.
    //  인자는 누가 불렀는지 한 줄(로그·종료 사유에 실린다). [why D-114]
    using ShutdownHandler = std::function<void(const std::string& who)>;
