...
{
public:
    explicit KisClient(const KisConfig& config);
    ~KisClient() override;
    ...
    {
        return !config_.account_no.empty();
    }
    ...
    // ── 파생 (선물·옵션) ────────────────────────────────────────────────────
    // 국내 선물/옵션 현재가 — inquire-price (거래ID transaction_id=FHMIF10000000).
    ...
    //  연속조회가 없어 요청으로 늘릴 수 없다(2026-09-23 실측).
    static constexpr int kRankingPageRows = 30;
...
private:
    ...
    // volume-rank 한 페이지(최대 kRankingPageRows행). 가격 구간을 주면 그 안에서만 순위를 매긴다.
