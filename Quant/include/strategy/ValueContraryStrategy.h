...
public:
    ...
    ValueContraryStrategy(Market      market,
                          std::string exchange,
                          double      pbr_max,
                          int         quantity,
                          int         eod_exit_hhmm)
    ...
    std::string id() const override {
        return std::string("VALUE_CONTRARY_")
             + (market_ == Market::KR ? "KR" : "US");
    }

    std::string describe() const override {
        return id()
             + " | PBR<=" + std::to_string(pbr_max_)
    ...
    void on_start() override {
        ...
        if (!kis_) { LOG_WARN("[ValueContrary] KisClient 없음"); return; }
        ...
        for (const auto& ticker : universe) {
            ...
            if ((int)bars.size() < 4) continue;

            // bars[0]=최근 완성 거래일, bars[1~3]=그 이전 3일
            bool declining = bars[0].close < bars[1].close
                          && bars[1].close < bars[2].close
                          && bars[2].close < bars[3].close;
            if (!declining) continue;

            candidates_.insert(ticker);
            LOG_INFO("[ValueContrary] 후보: " + ticker
                     + "  종가 " + std::to_string(bars[0].close)
                     + " < " + std::to_string(bars[1].close)
                     + " < " + std::to_string(bars[2].close));
        }
...
private:
                                                 ...
                                                 const std::string& time_str)
    {
        ...
        if (!is_in_session(hhmm)) return std::nullopt;
        ...
        if (buy_sent_.count(ticker)
            && !sell_sent_.count(ticker)
            && hhmm >= eod_exit_hhmm_)
        {
    ...
    static int parse_hhmm(const std::string& t) {
        ...
        try {
            return std::stoi(t.substr(0, 2)) * 100
                 + std::stoi(t.substr(2, 2));
        } catch (...) { return 0; }
    }
