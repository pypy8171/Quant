...
def run(sym, market_label, thr, mcap):
    ...
    for i0 in epos:
        ...
        if len(rows) >= 50:
            ...
            m = df["code"].map(mcap)
            sc = df["code"].map(snapclose)
            pit = m * np.exp(df["주가로그"]) / sc   # 스냅샷 시총에서 가격수익 채널을 되돌린다
            df["시총로그"] = np.log(pit.fillna(pit.median()))
            out.append(df)
    return out
...
A.to_csv(RAW / "crosssec_pit.csv", index=False, encoding="utf-8-sig")
