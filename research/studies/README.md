...
| [18_factor_harness/](18_factor_harness/README.md) | 저변동성 바스켓 — 250일(0.7)·20일(0.3) 실현변동성 하위 상위 30 동일가중·분기 리밸, 거래대금 하한 30억·50억, 23과 같은 세 층·7창·6/7 자. 유니버스는 재무 필터 없음 | **미달** — 하한 30억: 비용 MID 초과수익 연 +4.34%(t 0.72), 4/7창, 이웃 6/6; 50억: +6.36%(t 1.10), 5/7. MDD 36%(유니버스 52~55%)·샤프 0.35(0.05)로 위험 조정은 가설대로지만 초과수익 합격선 미달. 5분위 스프레드(t 4.2)는 고변동 최하위 분위가 지는 데서 나옴 — 롱 슬리브 편입 안 함, 두 슬리브의 고변동 제외 필터 단서로 남김 |
...
| [24_devscale_exit_lines/](24_devscale_exit_lines/README.md) | 모의계좌 DevScale 눌림 슬리브의 청산선(익절·손절·트레일)·하룻밤 넘김·전일 변동성 진입 필터를 1분봉 리플레이(상위 100종목, 2025-09-19~2026-09-18, 32,883 종목일, 왕복 비용 0.215%)로 격자 229개 | **채택**(D-111) — 당일 청산 안에서는 어떤 청산선도 세후 플러스 없음(15:15 강제 청산 1,862건). 넘김 + 익절 평단 3.0·손절 6.5·전일 ATR≤4%·개장 이격≥−1%가 세후 +0.342%/건, 387건/년, 13달 중 7달 플러스, +279만/년(투입 832만). 손절 6~7·익절 3 이상은 평평, ATR 5.5부터 절벽. 09-21 모의계좌 config 반영, 판정은 `scripts/check_runtime_health.py` 두 행 |
| [25_devscale_open_dev_rescue/](25_devscale_open_dev_rescue/README.md) | 스터디 24 후속 — D-111 채택값에서 개장 이격 조건(≥−1%)의 **재평가 시점**만 바꾼다. 09:03 단일 판정(현재 코드) vs 못 넘긴 날 10:00·10:30·11:00·12:00·13:30에 한 번 더 기회를 주는 구제. 같은 데이터·같은 나머지 파라미터, 변형 7개 | **기각** — 10:30 구제는 매수 387→390건, 세후 7,197→7,121원/건, 총액 278.5→277.7만(플러스 달 7/13 그대로). 이격 조건을 아예 꺼도 1년에 9건·−8.9만이 전부다. 진입을 막는 것은 이격이 아니라 정배열 존 게이트 — 32,883 종목일 중 후보가 1,284일이고 이격으로 막힌 날은 34일(2.6%) |
