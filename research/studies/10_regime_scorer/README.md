# 구조 국면 스코어러 제거실험 (연구군: index-macro)
...
- **가설**: 국면 판정을 이산({-2..+2})에서 연속·기울기로 바꾸면 하락장 진입/청산 타이밍이 개선되어, long/flat 프록시에서 매수 후 보유 대비 최대낙폭(MDD)를 줄이고 drawdown을 매끄럽게 만든다.
...
공개 표는 [`summary.tsv`](summary.tsv). 헤드라인 발췌(총수익% / MDD% / 샤프(위험조정수익) / 국면전환수, 매수 후 보유=매수 후 보유 지수):

| 창 | 변형 A(v0) | 변형 B(연속) | 변형 C(기울기) | 변형 D(오버레이) | 매수 후 보유(지수) |
|---|---|---|---|---|---|
...
- **국면 침묵 구간**: 2026now는 BEAR 판정이 없어 전 변형이 100% 노출 = 매수 후 보유와 동일(강세장에서 필터가 조용함 — 의도된 거동).
