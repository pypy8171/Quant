...
정직성:
 ...
 - 결측신호 → 중립(e=1) → 해당 전략은 그 구간 매수 후 보유와 동일(매매 0). 이걸 그대로 노출한다.

산출: journal_by_event.md(읽기용: 이벤트별 일별 베이스라인 + 15전략 매매내역 + 요약)
      journal_tsv/<event>.tsv(전체: 날짜×[close,매수 후 보유,15전략] 일별 평가액)
"""
import sys
...
def run(force_rep=None, tag=""):
    ...
    if force_rep:
        ...
        L.append("> ⚠️ **범위와 한계.** 한국 투자자가 **모든 위기를 KODEX 200(KOSPI200 근사)** 하나로 겪었다면. "
                 "15전략(M1~M5 방어 + C1~C5·O1~O5)은 전부 **지수 익스포저 오버레이**(개별종목 안 고름). "
                 "10억원을 KODEX 200에 넣고 전략이 **보유비중 e**(0=현금~1.2=완만레버)를 날마다 조절한다. "
                 "US신호(VIX·금리·유가·SOX·달러)는 **직전 미국세션값**으로 정렬(한국장이 먼저 마감하므로 시차를 차단). "
                 "익스포저는 t-1 종가까지 정보로만, 손익=e[t-1]×KOSPI수익, 비용 왕복 0.21%. "
                 "창 = **KOSPI 저점일 ±약2개월=총 4개월**. 저점 기준점은 창 중심잡기(평가)에만 씀. "
                 "**^KS11은 1996-12부터라 1929·1987·1990 위기는 KOSPI 데이터가 없어 제외한다.** "
                 "결측신호 전략은 그 구간 매수 후 보유와 동일(매매0). "
                 f"yfinance 무키. 생성 {TODAY}.\n")
    else:
        L.append("# 15전략 × 17이벤트 × 10억원 매매일지 (BT-09 부속)\n")
        L.append("> ⚠️ **범위와 한계.** 이 15전략(M1~M5 방어 + C1~C5·O1~O5)은 전부 **지수 익스포저 오버레이**다. "
                 ...
                 "창 = **저점일(climax) ±약2개월(거래일 42일씩)=총 4개월**. 저점 기준점은 창 중심잡기(평가)에만 씀. "
                 "KOSPI200은 1996-12부터라 그 이전 위기는 대표지수(미국)로 대체. 결측신호 전략은 그 구간 매수 후 보유와 동일(매매0). "
                 f"yfinance 무키. 생성 {TODAY}.\n")
    ...
    for eid, cause, rep0, d0, d1 in EVENTS:
        ...
        # 매수 후 보유 결과
        buy_and_hold_e = np.ones(len(closes))
        buy_and_hold_vals = sim_window(closes, ret, buy_and_hold_e, w_lo, w_hi)
        buy_and_hold_ret = (buy_and_hold_vals[-1] / CAP - 1.0) * 100.0
        buy_and_hold_mdd = max_drawdown(buy_and_hold_vals)
        L.append(f"- **그냥 보유(매수 후 보유) 10억 → {won(buy_and_hold_vals[-1])}원 ({fmt(buy_and_hold_ret,1)}%)**, 창내 최대낙폭(MDD) {fmt(buy_and_hold_mdd,1)}%")
        buy_and_hold_board.append(buy_and_hold_ret)

        # 일별 베이스라인 표
        L.append("\n### 일별 베이스라인 (지수 · 매수 후 보유 10억 평가액)\n")
        L.append("| 날짜 | 지수종가 | 일간% | 매수 후 보유 평가액(원) |")
        L.append("|---|---|---|---|")
        for k, dt in enumerate(wdates):
            r = (wcloses[k] / wcloses[k - 1] - 1.0) * 100.0 if k > 0 else 0.0
            L.append(f"| {dt} | {wcloses[k]:,.2f} | {fmt(r,2)} | {won(buy_and_hold_vals[k])} |")

        # 전략별 매매내역 + 요약
        summ = []
        tsv_cols = {"date": list(wdates), "close": [f"{close:.2f}" for close in wcloses],
                    "BUY_AND_HOLD": [f"{value:.0f}" for value in buy_and_hold_vals]}
        L.append("\n### 전략별 매매내역 (비중변경 = 실제 매매; 결측신호는 매매0=매수 후 보유동일)\n")
        ...
        # 이벤트 요약표
        L.append("\n### 이벤트 요약 (10억 → 최종평가액)\n")
        L.append("| 전략 | 구분 | 창내수익% | 창내MDD% | 매매횟수 | 진입비중% | 최종평가액(원) | vs 매수 후 보유 |")
        L.append("|---|---|---|---|---|---|---|---|")
        L.append(f"| 매수 후 보유 그냥보유 | 기준 | {fmt(buy_and_hold_ret,1)} | {fmt(buy_and_hold_mdd,1)} | 0 | 100 | {won(buy_and_hold_vals[-1])} | — |")
        for code, side, wret, wmdd, ntr, enter, fv in summ:
            L.append(f"| {code} | {side} | {fmt(wret,1)} | {fmt(wmdd,1)} | {ntr} | {fmt(enter*100,0)} | "
                     f"{won(fv)} | {fmt(wret-buy_and_hold_ret,1)}%p |")

        # TSV
        keys = ["date", "close", "BUY_AND_HOLD"] + [code for code, *_ in METHODS15]
        lines = ["\t".join(keys)]
        for i in range(len(wdates)):
            lines.append("\t".join(str(tsv_cols[k][i]) for k in keys))
        (tsv_dir / f"{eid}.tsv").write_text("\n".join(lines), encoding="utf-8")
        print(f"[ok] {eid}: {inst} {wdates[0]}~{wdates[-1]} 매수 후 보유 {fmt(buy_and_hold_ret,1)}%")

    # 종합 스코어보드
    L.append("\n\n## 전 이벤트 종합 스코어보드 (창내수익 평균·매수 후 보유초과 빈도)\n")
    L.append(f"매수 후 보유 평균 창내수익 {fmt(float(np.mean(buy_and_hold_board)),1)}% (n={len(buy_and_hold_board)}). "
             "아래는 각 전략의 17이벤트 평균·중앙값과 매수 후 보유를 이긴 이벤트 수.\n")
    L.append("| 전략 | 구분 | 평균수익% | 중앙값% | 매수 후 보유초과 이벤트 | 표본 |")
    L.append("|---|---|---|---|---|---|")
    ...
    L.append("\n*'창내수익'은 저점±2개월 4개월 구간 수익이라 대부분 음수(위기창)거나 반등포함. "
             "방어전략은 낙폭을 줄이는 대신 수익이 매수 후 보유보다 낮을 수 있고(정상), 공세전략은 반등참여로 초과를 노린다. "
             "매수 후 보유초과 이벤트 수가 절반 이상이면 그 창에서 값어치. 단 n≈13~17, 성격별로는 더 적어 과대해석 금지.*\n")
