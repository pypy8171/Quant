...
- 지금 도는 전략은 DeviationScale 하나입니다. 모의계좌에서 돌리고, 9월 하순에는 실계좌에서도 소액으로 돌렸습니다. 실계좌는 매수·매도 선점을 나누는 수정이 끝날 때까지 멈춰 두었습니다(2026-09-25~).
- 표본이 적어 수익성은 아직 판단하지 않습니다.
- 최근에는 엔진을 시세·전략·주문 세 프로세스로 나눌 수 있게 하고, 큰 파일을 역할별로 쪼갰습니다. 구조의 기준은 지금 규모(종목 수십 개·계좌 하나)가 아니라 전 시장 실시간 시세와 여러 계좌 대행 매매입니다(D-071·D-128).
...
C++ 엔진 (quant_trader) — 한 프로세스, 또는 --role 로 셋
    ...
    │ 수신 ×N       │ → │ 샤드 ×M·전략  │ → │ OrderGate 검증 → 주문     │
    │ (소켓 보유)   │   │               │ ← │ 체결 반영 → 원장          │
    └───────────────┘   └───────────────┘   └───────────────────────────┘
         └──────────── 공유 메모리 (시세·체결·요청·응답·제어·박동) ────┘
    │                                        │
