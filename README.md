...
백테스트로 거를 수 없는 전략(분봉 단위 등 과거 시점정합 재현 불가)은 **모의계좌 forward 실증**이 유일한 검증 경로입니다. 매일의 존 판정·발주·체결·거부를 하루 1파일로 남겨, 전략이 의도대로 동작했는지와 백테스트 가정이 실제 체결과 어디서 어긋나는지를 사후에 대조합니다.

- 전략별 인덱스: [strategies/README.md](strategies/README.md)
- 최근 실증: [DeviationScale 2026-08-13](strategies/DeviationScale/live/2026-08-13.md) · [ITB 2026-08-07](strategies/ITB/live/2026-08-07.md)
- 원시 로그(`*.log`)·손익 CSV는 재생성 가능한 노이즈라 커밋하지 않고, **해석이 담긴 일지 md만** 남깁니다.
