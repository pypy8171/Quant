...
#   - 순서는 고정: EOD AutoDoc → Maintain Daily → Minute Backfill → stock_study → dashboard_sync → Maintain Weekly(금).
param(
    [string]$Config = $null,
    [switch]$Apply,
    [switch]$Lines,
...
$fix = @{
    'Quant EOD AutoDoc'     = 'py scripts\eod_autodoc.py'
    'Quant Maintain Daily'  = 'py scripts\maintain.py --daily'
    'Quant Minute Backfill' = 'py scripts\eod_minute_backfill.py'
...
if (-not $Apply)
{
    if ($drift -gt 0)
    {
        Write-Host "[eod_timetable] 어긋남 $drift 건 — -Apply 로 맞춘다"
        exit 1
    }

    Write-Host "[eod_timetable] 시간표 일치"
    exit 0
}
...
foreach ($name in $plan.Keys)
{
    ...
    {
        Write-Host "[eod_timetable] schtasks /change 실패: $name"
        exit 1
    }
...
Write-Host "[eod_timetable] 적용 끝 — .claude/hooks/cron-gate.ps1 는 이 스크립트의 -Lines 를 읽으므로 따로 고칠 것 없다"
exit 0
