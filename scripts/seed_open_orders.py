#!/usr/bin/env python3
"""실행 로그에서 미체결 주문을 복원해 open_orders.txt 사이드카를 채운다.

평시에는 필요 없다 — OrderRouter가 접수·체결마다 사이드카를 직접 쓰고, 기동 때
cancel_stale_orders()가 그것을 읽어 전부 취소한다. 이 스크립트는 두 경우의 복구용이다.

  1. 사이드카가 없던 시절(2026-09-08 이전) 실행분의 유령 주문 정리
  2. 사이드카가 유실됐는데 로그는 남아 있는 경우
