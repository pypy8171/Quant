...
RULES = [
    ...
    (r"B&H", "매수 후 보유", "B&H"),
    (r"\bBH\b(?!-FDR)", "매수 후 보유", "BH(산문)"),  # BH-FDR(다중검정 보정)은 통계 용어라 뺀다
    # 등가중 → 동일가중 통일
    (r"등가중", "동일가중", "등가중"),
    # 코인드 문구
    ...
    (r"껍데기", "빈 창", "껍데기"),
    (r"잔재", "남은 프로세스", "잔재"),
    # 커밋 문체 스캔이 반복 적발한 둘(2026-09-12). 정본 docs/STYLE_GUIDE.md의 대체어를 그대로 쓴다.
    (r"좌초", "미완료", "좌초"),
    (r"신선도", "갱신 상태", "신선도"),
    (r"굶었", "밀렸", "굶었다"),
    (r"굶는", "밀리는", "굶는"),
    (r"두들긴다", "반복 호출한다", "두들긴다"),
    (r"두들기", "반복 호출하", "두들기"),
    # 문서가 코드를 가리키는 뜻이면 "줄번호 참조"가 맞다 — --fix 뒤 눈으로 고른다(정본 docs/STYLE_GUIDE.md 50~51행). 2026-09-19
    (r"앵커", "기준점", "앵커"),
    # H7 눌림 필터 가설 코드
