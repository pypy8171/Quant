...
# 로그 라인:  2026-09-03 09:53:49.011 {Order thread} [INFO ] [태그] 메시지  ({스레드}는 선택, scripts/parse_quant_log.py와 같은 규약)
...
async function tick(){
  ...
  if(r.__error__){ rd.innerHTML='<small class="err">'+eb(r.__error__)+'</small>'; }
    ...
    //  코스피·코스닥은 premarket 얼어붙기로 채운다. 나스닥·S&P500은 현물 직전 세션(어제)에 선물 정산 후 변동을
    //  더한 값이 pct라, 두 조각(어제·정산 후)을 옆에 보인다.
    const prevp=(c)=>(c.prev_pct!=null?` <span class="mut">어제 ${pct(c.prev_pct)}</span>`:'')
      +(c.since_settle_pct!=null?` <span class="mut">정산 후 ${pct(c.since_settle_pct)}</span>`:'');
