...
7. 성능을 이유로 고칠 때만 먼저 잰다. 가독성·구조 리팩터는 수치 없이 한다 — 조건은 동작 불변(ctest 통과)과, hot path를
   건드리면 전후 측정을 같이 붙이는 것.
...
엔드포인트·TR ID·전문 필드표·제약값은 **MCP 코딩도우미 `kis-code-assistant`를 먼저 부른다** — 기억·주석·옛 문서보다 최신이다.
`search_*_api`로 찾고 `read_source_code`로 공식 샘플을 읽는다. 거기에 없을 때만 저장소 문서·웹을 본다. 확인한 값은 쓰는 자리에
확인일과 함께 남긴다. 설정은 `.mcp.json`(실행에 `uv` 필요, 인증정보 안 씀). 샘플이 제도 변경보다 늦을 수 있어 마감은 실측이다.

## 장중 운영 — A등급 결함만 장중에 고친다 (D-101 결정 1)

매매 창(평일 09:00~15:30, 실계좌 20:00) 안의 exe 교체·재기동은 **A등급 결함(체결 누락·이중 발주·원장 불일치·주문 불능)** 일 때만.
그때는 묻지 말고 수정 → 테스트 → 재빌드 → 재기동(되묻는 사이 체결이 원장에서 새는 것을 이미 겪었다). 리팩터·이름·문서·성능은
장 마감 뒤 배포 — 재기동 42회 중 29회가 그런 이유의 세션 kill이었고 이관 청산이 −198만/주였다. `scripts/deploy_guard.py`가
장중 재링크를 막는다(A등급은 `--hotfix-a "사유"`). 재기동은 **repo 루트에서 원래 인자 그대로**, 트레이더는 감시견
