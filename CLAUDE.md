...
고쳐서 테스트를 통과했으면 시각을 가리지 않고 배포한다(사용자 지시 2026-09-23, 옛 D-101 결정 1의 시각 제한은 걷었다).
묻지 말고 수정 → 테스트 → 재빌드 → 재기동. `scripts/deploy_guard.py`는 막지 않고 매매 창 안의 교체를
`_private/deploy_guard.log`에 한 줄 남기기만 한다 — 재기동은 보유분을 청산 래퍼로 넘기므로(재기동 42회 중 29회·−198만/주)
몇 번 그랬는지는 남긴다. 재기동은 **repo 루트에서 원래 인자 그대로**, 트레이더는 감시견
