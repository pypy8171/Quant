...
| 워치독 | `scripts/auto_trade_day.ps1` | 사전 점검(중복 프로세스·계좌 모드), 기동 전 `quant_trader` 재빌드(증분, 실패면 `build_failed`로 중단, `-NoBuild`로 생략), 보조 프로세스·유니버스·대시보드·알림·체결 기록기(`quant-recorder`)·엔진 자원 표본기(`quant-procwatch`, 리눅스 트레이더면 `--wsl-distro Ubuntu-24.04`)·원장 저널 적재기(`quant-ledger`, `PYQuant/tools/ledger_recorder.py --dir <ledger_journal_dir>`, D-113) 기동, KIS 토큰 캐시를 `KIS_TOKEN_CACHE_DIR`로 트레이더와 한 파일로 맞춤, 트레이더를 마감까지 감시·재기동, 마감 뒤 `market_close_autodoc.py` 실행. `-Split`을 주면 트레이더를 주문 쪽·전략 쪽 두 프로세스로 띄우고, 한쪽이 내려가면 짝도 내려 둘을 같이 다시 띄운다(공유 쪽지가 옛 값인 채 남는 것을 막는다, D-114) |
...
트레이더를 내릴 때는 **곱게 먼저**다 — `Quant/build_win/ops_client.exe ... shutdown <세션 이름>`을 청해
25초까지 기다렸다가, 그 안에 안 내려간 pid 만 `taskkill /F`로 내린다(D-114). 곱게 내려가면 `stop()`이 돌아
공유 쪽지의 종료 사유가 채워지고, 그래서 다음 기동이 앞선 끝을 크래시로 읽지 않는다. `ops_client kill`은 쓰지
않는다 — 킬스위치 표지를 남겨 그날 내내 재기동이 막힌다. 주문 쪽이 곱게 나가면 전략 쪽도 5초 안에 그걸 보고
따라 나가고, 감시견은 그동안 20초를 기다렸다가 그래도 안 나가면 강제로 내린다(짝이 크래시였으면 기다리지 않는다).
