...
| 고아 프로세스, 잔재 | 남은 프로세스 |
| 스모크 프로브, 기동 프로브, 프로브, probe | 기동 점검, 점검 |
...
커밋 제목은 `type(범위): 한국어 제목`이다(Conventional Commits 관행). type은 이 표의 것만 쓰고, 새 type이 필요하면 여기에 먼저 한 줄 추가한다 —
뜻을 찾아볼 곳은 이 표 하나다.

| type | 뜻 | 쓰는 때 |
|---|---|---|
| `feat` | 기능 | 새 동작·설정·전략 추가 |
| `fix` | 수정 | 잘못된 동작을 고침 |
| `refactor` | 정리 | 동작은 그대로 두고 구조·이름·주석만 바꿈 |
| `docs` | 문서 | 문서·가이드·결정 기록(D-NNN) |
| `test` | 시험 | 테스트 추가·수정 |
| `bench` | 측정 | 벤치·지연 측정 코드와 그 결과 |
| `build` | 빌드 | CMake·의존성·빌드 스크립트 |
| `chore` | 잡무 | 위에 안 드는 살림 — 하네스(`.claude/`)·훅·예약작업·유지보수 스크립트 |

범위(괄호)는 폴더·영역 이름이다: `core`·`engine`·`strategy`·`api`·`risk`·`docs`·`harness`·`automation`·`research`. 예: `refactor(core): 틱·호가·봉 구조체의 종목 코드를 고정 배열 sym::Ticker로 바꾼다 (D-071)`.
