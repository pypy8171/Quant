...
- [.mcp.json](../.mcp.json) — MCP 서버 설정(프로젝트 스코프). KIS 코딩도우미 kis-code-assistant — 인증정보 없음(D-120)
...
- [load_test.json](../PYQuant/grafana/provisioning/dashboards/json/load_test.json) — (설명 필요)
...
- [KisRateBucket.h](../Quant/include/api/KisRateBucket.h) — KIS REST 초당 한도 토큰 버킷 크기 계산(헤더 전용 constexpr)
...
- [test_kis_rate_bucket.cpp](../Quant/tests/test_kis_rate_bucket.cpp) — KIS REST 초당 한도 버킷 단위 테스트(공표 한도 초과·요구 토큰 과다)
...
- [aftermarket_feed_check.py](../scripts/aftermarket_feed_check.py) — 애프터마켓(16:00~20:00) 체결이 기존 WS 채널로 오는지 FEED 모드로 실측한다
...
- [check_market_open.py](../scripts/check_market_open.py) — 그날 장이 열리는지 KIS 휴장일조회로 판정한다(0=개장 1=휴장 2=모름). 감시견의 휴장일 관문
