...
4. [`Engine::start`](../Quant/src/core/Engine.cpp#L531) — 행렬 `reshape`(행=수신 레인+폴러, 열=샤드) → 샤드 생성 → 원장 시드 → WS 연결·콜백 → jthread 다섯. 아래 두 걸음은 이 함수 안이다  
   `Quant/src/core/Engine.cpp:531` · `void Engine::start()`
5. [`Engine::start (WS 콜백 설치)`](../Quant/src/core/Engine.cpp#L844) — 소켓 레인 i의 호가·체결 콜백. 종목 id로 열을 고르고(`consumer_of`) 자기 행에 `push_to` — 가득 차면 버리고 센다(블로킹 금지)  
   `Quant/src/core/Engine.cpp:844` · `ws_->set_lane_callbacks([this] (uint32_t lane, const OrderBook& in) …`
6. [`Engine::start (스레드 기동)`](../Quant/src/core/Engine.cpp#L960) — data·strategy·order·fill·control 다섯 jthread + 샤드 M. stop_token이 첫 인자라 람다로 감싼다  
   `Quant/src/core/Engine.cpp:960` · `data_thread_     = std::jthread([this] (std::stop_token st) { data_thread_fn(st); });`
...
16. [`Engine::data_thread_fn`](../Quant/src/core/Engine.cpp#L1152) — REST 축 — 봉 폴링·국면 재평가·잔고 대조·유니버스 재스캔. 폴러의 대체 틱은 `td_mx_`의 자기 행(`data_row_`)으로 간다  
   `Quant/src/core/Engine.cpp:1152` · `void Engine::data_thread_fn(std::stop_token st)`
...
19. [`Engine::shard_thread_fn`](../Quant/src/core/Engine.cpp#L2016) — 전략 집합 버전이 바뀌면 `rebuild`, 아니면 `step`. 비면 `WakeGate`로 잠든다. `emit`은 `shard_out_` push + 디스패치 스레드 깨우기  
   `Quant/src/core/Engine.cpp:2016` · `void Engine::shard_thread_fn(std::stop_token st, uint32_t m)`
...
29. [`Engine::strategy_thread_fn`](../Quant/src/core/Engine.cpp#L1809) — 루프 한 바퀴: `poll_regime_file` → `flush_held` → 강제청산 → `trim_excess_once` → `shard_out_` 비우기(`from_strategy`) → 수동 주문 → 잠  
   `Quant/src/core/Engine.cpp:1809` · `void Engine::strategy_thread_fn(std::stop_token st)`
30. [`Engine::poll_regime_file`](../Quant/src/core/Engine.cpp#L1727) — `regime.json` 축 — `entry_halt`(신규매수 차단)·`entry_scale`(매수비율)·`force_liquidate`, 그리고 라벨 전이 때 `apply_regime_selection`(전략 집합 선택, D-084). 상태기계는 `RegimeFileBridge.h`  
   `Quant/src/core/Engine.cpp:1727` · `void Engine::poll_regime_file()` · 시험 [test_regime_bridge](../Quant/tests/test_regime_bridge.cpp)
...
34. [`Engine::drain_manual_inbox`](../Quant/src/core/Engine.cpp#L2618) — 운영단말 수동 주문이 같은 싱크로 들어온다 — 생산자를 늘리지 않기 위해 이 스레드가 꺼낸다  
   `Quant/src/core/Engine.cpp:2618` · `void Engine::drain_manual_inbox(const std::function<void(const OrderSignal&)>& emit)`
...
35. [`Engine::order_thread_fn`](../Quant/src/core/Engine.cpp#L2144) — `take_due_retry` 우선 → 큐 pop → `wait_before_send` → `router->submit` → 성공이면 `note_sent`, 거부면 `on_rejected` → `LatencyTrace::record`. 비면 재시도 만기까지 `wait_until`  
   `Quant/src/core/Engine.cpp:2144` · `void Engine::order_thread_fn(std::stop_token st)`
...
47. [`Engine::fill_thread_fn`](../Quant/src/core/Engine.cpp#L2250) — `fill_queue_` pop → `on_fill` → `ledger_->note_fill`(대조 5초 유예, D-074) → 운영단말 `broadcast`. 비면 `WakeGate`  
   `Quant/src/core/Engine.cpp:2250` · `void Engine::fill_thread_fn(std::stop_token st)`
...
50. [`Engine::control_thread_fn`](../Quant/src/core/Engine.cpp#L2403) — 큐 고수위 1분 로그 → 토큰 선갱신(5분, 만료 30분 전) → 손익 갱신 감시(끊기면 보수정지) → WS stale·재연결·REST 폴백  
   `Quant/src/core/Engine.cpp:2403` · `void Engine::control_thread_fn(std::stop_token st)`
...
52. [`Engine::maybe_rescan_universe`](../Quant/src/core/Engine.cpp#L256) — 유니버스 재스캔 — 빠진 보유 종목은 40초에 신규매수 차단, 600초에 전략 해제(`UniverseExit.h`, D-077)  
   `Quant/src/core/Engine.cpp:256` · `void Engine::maybe_rescan_universe()` · 시험 [test_signal_dispatcher](../Quant/tests/test_signal_dispatcher.cpp)
...
54. [`Engine::activate_rest_fallback`](../Quant/src/core/Engine.cpp#L2365) — WS가 stale이면 REST 현재가 폴링으로 대체 틱(`recv_ns`=0). 복귀는 `deactivate_rest_fallback`  
   `Quant/src/core/Engine.cpp:2365` · `bool Engine::activate_rest_fallback(const std::string& reason)`
...
55. [`Engine::request_shutdown`](../Quant/src/core/Engine.cpp#L1059) — 시그널 핸들러에서 불려도 되는 최소 동작(플래그·깨우기)만  
   `Quant/src/core/Engine.cpp:1059` · `void Engine::request_shutdown()`
56. [`Engine::stop`](../Quant/src/core/Engine.cpp#L1074) — stop_token 요청 → join 순서(수신 먼저, 체결 마지막) → 통계 출력. 미체결 예약주문 기억은 여기서 사라진다(재기동 규칙, CLAUDE.md '장중 운영')  
   `Quant/src/core/Engine.cpp:1074` · `void Engine::stop()`
