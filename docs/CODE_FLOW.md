...
- [6. 제어·주기 — 파이프라인 밖에서 돌아가는 것](#6-제어주기-파이프라인-밖에서-돌아가는-것) — 6걸음
...
1. [`main`](../Quant/src/main.cpp#L139) — 진입. `load_config` → `Engine engine(...)` → `set_*` 배선 → `load_strategies` → `engine.start()` 순서를 훑는다  
   `Quant/src/main.cpp:139` · `int main(int argc, char* argv[])`
...
4. [`Engine::start`](../Quant/src/core/Engine.cpp#L528) — 행렬 `reshape`(행=수신 레인+폴러, 열=샤드) → 샤드 생성 → 원장 시드 → WS 연결·콜백 → jthread 다섯. 아래 두 걸음은 이 함수 안이다  
   `Quant/src/core/Engine.cpp:528` · `void Engine::start()`
5. [`Engine::start (WS 콜백 설치)`](../Quant/src/core/Engine.cpp#L845) — 소켓 레인 i의 호가·체결 콜백. 종목 id로 열을 고르고(`consumer_of`) 자기 행에 `push_to` — 가득 차면 버리고 센다(블로킹 금지)  
   `Quant/src/core/Engine.cpp:845` · `ws_->set_lane_callbacks([this] (uint32_t lane, const OrderBook& in) …`
6. [`Engine::start (스레드 기동)`](../Quant/src/core/Engine.cpp#L961) — data·strategy·order·fill·control 다섯 jthread + 샤드 M. stop_token이 첫 인자라 람다로 감싼다  
   `Quant/src/core/Engine.cpp:961` · `data_thread_     = std::jthread([this] (std::stop_token st) { data_thread_fn(st); });`
...
16. [`Engine::data_thread_fn`](../Quant/src/core/Engine.cpp#L1153) — REST 축 — 봉 폴링·국면 재평가·잔고 대조·유니버스 재스캔. 폴러의 대체 틱은 `td_mx_`의 자기 행(`data_row_`)으로 간다  
   `Quant/src/core/Engine.cpp:1153` · `void Engine::data_thread_fn(std::stop_token st)`
...
19. [`Engine::shard_thread_fn`](../Quant/src/core/Engine.cpp#L2000) — 전략 집합 버전이 바뀌면 `rebuild`, 아니면 `step`. 비면 `WakeGate`로 잠든다. `emit`은 `shard_out_` push + 디스패치 스레드 깨우기  
   `Quant/src/core/Engine.cpp:2000` · `void Engine::shard_thread_fn(std::stop_token st, uint32_t m)`
...
24. [`DeviationScaleStrategy::on_start`](../Quant/include/strategy/DeviationScaleStrategy.h#L231) — 종목 id 받기·REST 봉 시드·프리페치 스레드. 전략 하나를 끝까지 따라가는 예로 이 전략을 쓴다  
   `Quant/include/strategy/DeviationScaleStrategy.h:231` · `void on_start() override`
25. [`DeviationScaleStrategy::on_trade_batch`](../Quant/include/strategy/DeviationScaleStrategy.h#L288) — 틱 → `agg_.on_tick` → 판단 직전 `close_stale`·`bars::resample` → 진입/청산 판단 → out. 매매 로직의 본체  
   `Quant/include/strategy/DeviationScaleStrategy.h:288` · `void on_trade_batch(const TradeData& td, std::vector<OrderSignal>& out) override`
...
28. [`DeviationScaleStrategy::emit_liquidation`](../Quant/include/strategy/DeviationScaleStrategy.h#L1212) — 청산 신호 조립 — 시장가면 `ref_price` 스탬프, 매도 가능 수량은 원장 접근자(`sellable_qty`, 동기 잔고조회 금지)  
   `Quant/include/strategy/DeviationScaleStrategy.h:1212` · `bool emit_liquidation(std::vector<OrderSignal>& out, int pos, …`
...
29. [`Engine::strategy_thread_fn`](../Quant/src/core/Engine.cpp#L1808) — 루프 한 바퀴: `poll_regime_file` → `flush_held` → 강제청산 → `trim_excess_once` → `shard_out_` 비우기(`from_strategy`) → 수동 주문 → 잠  
   `Quant/src/core/Engine.cpp:1808` · `void Engine::strategy_thread_fn(std::stop_token st)`
30. [`Engine::poll_regime_file`](../Quant/src/core/Engine.cpp#L1746) — `regime.json` 축 — `entry_halt`(신규매수 차단)·`force_liquidate`. 상태기계는 `RegimeFileBridge.h`  
   `Quant/src/core/Engine.cpp:1746` · `void Engine::poll_regime_file()` · 시험 [test_regime_bridge](../Quant/tests/test_regime_bridge.cpp)
...
33. [`dispatch::SignalDispatcher::force_liquidate`](../Quant/src/core/SignalDispatcher.cpp#L240) — 보유 전량 시장가 매도를 2초 간격 재발주. `ref_price`가 여기서 찍히는지 본다  
   `Quant/src/core/SignalDispatcher.cpp:240` · `void SignalDispatcher::force_liquidate(Clock::time_point now)` · 시험 [test_signal_dispatcher](../Quant/tests/test_signal_dispatcher.cpp)
34. [`Engine::drain_manual_inbox`](../Quant/src/core/Engine.cpp#L2602) — 운영단말 수동 주문이 같은 싱크로 들어온다 — 생산자를 늘리지 않기 위해 이 스레드가 꺼낸다  
   `Quant/src/core/Engine.cpp:2602` · `void Engine::drain_manual_inbox(const std::function<void(const OrderSignal&)>& emit)`
...
35. [`Engine::order_thread_fn`](../Quant/src/core/Engine.cpp#L2128) — `take_due_retry` 우선 → 큐 pop → `wait_before_send` → `router->submit` → 성공이면 `note_sent`, 거부면 `on_rejected` → `LatencyTrace::record`. 비면 재시도 만기까지 `wait_until`  
   `Quant/src/core/Engine.cpp:2128` · `void Engine::order_thread_fn(std::stop_token st)`
...
38. [`OrderRouter::submit`](../Quant/src/ipc/OrderRouter.cpp#L48) — `gate_.check` → `kis_.submit_order_ack`(ODNO·KRX 조직번호) → `gate_.on_accept` → 원장 CSV 행(`seq` 동반). 실패 경로마다 무엇이 되돌려지는지  
   `Quant/src/ipc/OrderRouter.cpp:48` · `ManagedOrder OrderRouter::submit(const OrderSignal& sig)` · 시험 [test_order_router](../Quant/tests/test_order_router.cpp)
...
41. [`OrderGate::plan_displacement`](../Quant/src/risk/OrderGate.cpp#L1143) — 슬롯이 찼을 때 어느 보유를 내보낼지. 디스패처의 교체 진입이 이 계획을 쓴다  
   `Quant/src/risk/OrderGate.cpp:1143` · `OrderGate::DisplacePlan OrderGate::plan_displacement(const std::string& account, …` · 시험 [test_order_gate](../Quant/tests/test_order_gate.cpp)
...
43. [`KisClient::submit_order_ack`](../Quant/src/api/KisOrder.cpp#L129) — 현금 주문 REST. tr_id(실/모의)·`auth_headers`·응답에서 ODNO. 여기서만 KIS에 주문이 닿는다  
   `Quant/src/api/KisOrder.cpp:129` · `OrderAck KisClient::submit_order_ack(const OrderSignal& signal)`
...
47. [`Engine::fill_thread_fn`](../Quant/src/core/Engine.cpp#L2234) — `fill_queue_` pop → `on_fill` → `ledger_->note_fill`(대조 5초 유예, D-074) → 운영단말 `broadcast`. 비면 `WakeGate`  
   `Quant/src/core/Engine.cpp:2234` · `void Engine::fill_thread_fn(std::stop_token st)`
48. [`OrderRouter::on_fill`](../Quant/src/ipc/OrderRouter.cpp#L1495) — ODNO로 주문 찾기 → 상태 갱신 → `gate_.on_fill_confirmed` → 원장 CSV. 못 찾으면 미연결 체결 경로  
   `Quant/src/ipc/OrderRouter.cpp:1495` · `void OrderRouter::on_fill(const FillNotification& fn)` · 시험 [test_order_router](../Quant/tests/test_order_router.cpp)
...
50. [`Engine::control_thread_fn`](../Quant/src/core/Engine.cpp#L2387) — 큐 고수위 1분 로그 → 토큰 선갱신(5분, 만료 30분 전) → 손익 갱신 감시(끊기면 보수정지) → WS stale·재연결·REST 폴백  
   `Quant/src/core/Engine.cpp:2387` · `void Engine::control_thread_fn(std::stop_token st)`
51. [`RegimeController::evaluate`](../Quant/src/core/RegimeController.cpp#L36) — 지수 200MA·정배열 점수 → BULL/NEUTRAL/BEAR. 오늘 봉 접기(`fold_today`)·전환 확인 `confirm_n`(D-076). 데이터 스레드 단일 호출자  
   `Quant/src/core/RegimeController.cpp:36` · `RegimeSnapshot RegimeController::evaluate()` · 시험 [test_regime](../Quant/tests/test_regime.cpp)
52. [`Engine::apply_regime_selection`](../Quant/src/core/Engine.cpp#L158) — 국면 → `regime_strategies` 집합으로 전략 활성/비활성. 청산은 하지 않는다  
   `Quant/src/core/Engine.cpp:158` · `void Engine::apply_regime_selection(Regime r, bool force_log)`
53. [`Engine::maybe_rescan_universe`](../Quant/src/core/Engine.cpp#L253) — 유니버스 재스캔 — 빠진 보유 종목은 40초에 신규매수 차단, 600초에 전략 해제(`UniverseExit.h`, D-077)  
   `Quant/src/core/Engine.cpp:253` · `void Engine::maybe_rescan_universe()` · 시험 [test_signal_dispatcher](../Quant/tests/test_signal_dispatcher.cpp)
54. [`ledger::LedgerReconciler::reconcile`](../Quant/src/core/LedgerReconciler.cpp#L277) — 브로커 잔고 ↔ 원장. 어긋난 종목만 `RECONCILE` 행(`ReconcilePlan.h` 순수 함수). 잔고조회 서킷브레이커  
   `Quant/src/core/LedgerReconciler.cpp:277` · `void LedgerReconciler::reconcile(bool resync_positions, std::time_t now_utc)` · 시험 [test_ledger_reconciler](../Quant/tests/test_ledger_reconciler.cpp)
55. [`Engine::activate_rest_fallback`](../Quant/src/core/Engine.cpp#L2349) — WS가 stale이면 REST 현재가 폴링으로 대체 틱(`recv_ns`=0). 복귀는 `deactivate_rest_fallback`  
   `Quant/src/core/Engine.cpp:2349` · `bool Engine::activate_rest_fallback(const std::string& reason)`

리뷰할 때 볼 것:

- 제어 작업이 파이프라인 락을 잡는지 — 잡으면 최악 지연이 여기서 난다
- 국면 축 둘의 역할이 섞이지 않는지 — `RegimeController`는 전략 집합만, 청산은 `regime.json` 축만
...
56. [`Engine::request_shutdown`](../Quant/src/core/Engine.cpp#L1060) — 시그널 핸들러에서 불려도 되는 최소 동작(플래그·깨우기)만  
   `Quant/src/core/Engine.cpp:1060` · `void Engine::request_shutdown()`
57. [`Engine::stop`](../Quant/src/core/Engine.cpp#L1075) — stop_token 요청 → join 순서(수신 먼저, 체결 마지막) → 통계 출력. 미체결 예약주문 기억은 여기서 사라진다(재기동 규칙, CLAUDE.md '장중 운영')  
   `Quant/src/core/Engine.cpp:1075` · `void Engine::stop()`

## 부록. 실계좌 없이 같은 경로를 돌리는 것

리플레이 소스와 모의 체결기가 소켓·KIS 자리에 들어간다. 위 흐름을 캡처 파일로 다시 밟을 때 본다.

58. [`ReplaySource`](../Quant/include/core/ReplaySource.h#L21) — 캡처 파일 → `IFeedSource`. 내보낼 때 `recv_ns`를 다시 찍는다. config `replay_file`·`replay_speed`  
...
59. [`PaperExecutor`](../Quant/include/core/PaperExecutor.h#L27) — `IOrderExecutor` 모의 체결기 — 주문 즉시 체결통보를 만들어 같은 `fill_queue_` 경로로 넣는다  
...
60. [`sync::WakeGate`](../Quant/include/core/WakeGate.h#L16) — 소비자 잠·깨우기 한 조각. 전략·주문·체결·샤드 유휴가 전부 이걸 쓴다  
...
| [Quant/include/core/PaperExecutor.h](../Quant/include/core/PaperExecutor.h) | 59 `PaperExecutor` |
| [Quant/include/core/ReplaySource.h](../Quant/include/core/ReplaySource.h) | 58 `ReplaySource` |
...
| [Quant/include/core/WakeGate.h](../Quant/include/core/WakeGate.h) | 60 `WakeGate` |
...
| [Quant/src/core/Engine.cpp](../Quant/src/core/Engine.cpp) | 3 `add_strategy`, 4 `start`, 5 `start (WS 콜백 설치)`, 6 `start (스레드 기동)`, 16 `data_thread_fn`, 19 `shard_thread_fn`, 29 `strategy_thread_fn`, 30 `poll_regime_file`, 34 `drain_manual_inbox`, 35 `order_thread_fn`, 47 `fill_thread_fn`, 50 `control_thread_fn`, 52 `apply_regime_selection`, 53 `maybe_rescan_universe`, 55 `activate_rest_fallback`, 56 `request_shutdown`, 57 `stop` |
| [Quant/src/core/LedgerReconciler.cpp](../Quant/src/core/LedgerReconciler.cpp) | 7 `bootstrap`, 54 `reconcile` |
| [Quant/src/core/OrderPacer.cpp](../Quant/src/core/OrderPacer.cpp) | 36 `wait_before_send`, 37 `on_rejected` |
| [Quant/src/core/RegimeController.cpp](../Quant/src/core/RegimeController.cpp) | 51 `evaluate` |
