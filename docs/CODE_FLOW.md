...
flowchart LR
  ...
  subgraph fill[5. 체결 스레드]
    FN[parse_fill_notification] --> FQ[pipeline_.fill_queue SPSC] --> OF[OrderRouter::on_fill] --> OGF[PositionLedger::on_fill_confirmed]
  end
  KIS -. ODNO .-> OR
  WS -. H0STCNI9 .-> FN
```
...
5. [`load_strategies`](../Quant/src/strategy/StrategyFactory.cpp#L1116) — config `strategies[]`를 전략 객체로. 새 전략을 붙이는 자리(docs/ENGINE_ARCHITECTURE.md '전략 추가하기')  
   `Quant/src/strategy/StrategyFactory.cpp:1116` · `void load_strategies(StrategyLoadCtx& context, const json& strategies)`
...
7. [`Engine::start`](../Quant/src/core/Engine.cpp#L482) — 도우미 호출 목록이 기동 순서다 — `setup_shards`(행렬 `reshape`, 행=수신 스레드+폴러, 열=샤드) → ZMQ → 모의 체결기 → 라우터·대조기·폴러 → `try_bootstrap_ledger`(원장 시드) → `start_strategies` → 구독 목록 → `connect_feed` → `spawn_threads`  
   `Quant/src/core/Engine.cpp:482` · `void Engine::start()`
8. [`Engine::connect_feed (WS 콜백 설치)`](../Quant/src/core/EngineFeed.cpp#L582) — 소켓 수신 스레드 i의 호가·체결 콜백. 종목 id로 열을 고르고(`consumer_of`) `pipeline_.trade_matrix`·`order_book_matrix`의 자기 행에 `push_to` — 가득 차면 버리고 센다(블로킹 금지)  
   `Quant/src/core/EngineFeed.cpp:582` · `feed_.websocket->set_lane_callbacks([this] (uint32_t lane, const OrderBook& in) …`
9. [`Engine::spawn_threads`](../Quant/src/core/Engine.cpp#L447) — data·strategy·order·fill·control 다섯 jthread + 샤드 M. stop_token이 첫 인자라 람다로 감싼다  
...
13. [`KisWebSocket::dispatch_record`](../Quant/src/api/KisWebSocketParse.cpp#L262) — tr_id로 채널 분기 — H0STCNT0 체결·H0STASP0 호가(KRX), H0UNCNT0/H0UNASP0(KRX+NXT 통합, D-096), H0STCNI0/H0STCNI9 체결통보(실/모의)  
...
14. [`KisWebSocket::parse_kr_trade`](../Quant/src/api/KisWebSocketParse.cpp#L336) — `decode_kr_trade` → `trade.symbol_id`(SymbolTable) → `received_ns` 스탬프 → `on_trade_` 콜백. 호가는 `parse_orderbook`이 같은 모양  
   `Quant/src/api/KisWebSocketParse.cpp:336` · `void KisWebSocket::parse_kr_trade(kis_websocket::Fields fields)`
15. [`kis_websocket::decode_kr_trade`](../Quant/include/api/KisWsDecode.h#L135) — 순수 함수. 필드 인덱스 → `TradeData`(가격·수량·`hhmmss` 정수·방향). 필드 번호가 [wire] 정본  
...
16. [`kis_websocket::decode_orderbook`](../Quant/include/api/KisWsDecode.h#L130) — 5단계 호가 → `OrderBook`. 매도·매수 가격/잔량 필드 위치  
...
20. [`DataPoller::poll_universe`](../Quant/src/core/DataPoller.cpp#L88) — REST 현재가 → 대체 `TradeData`(`received_ns`=0). 구독 상한 넘침·틱 끊긴 보유 보충(`top_up`)도 이 클래스  
...
22. [`Engine::shard_thread_fn`](../Quant/src/core/EngineStrategyThread.cpp#L410) — 전략 집합 버전이 바뀌면 `rebuild`, 아니면 `step`. 비면 `WakeGate`로 잠든다. `emit`은 `pipeline_.shard_out` push + 디스패치 스레드 깨우기  
...
27. [`DeviationScaleStrategy::on_start`](../Quant/include/strategy/DeviationScaleStrategy.h#L192) — 종목 id 받기·REST 봉 시드·프리페치 스레드. 전략 하나를 끝까지 따라가는 예로 이 전략을 쓴다  
   `Quant/include/strategy/DeviationScaleStrategy.h:192` · `void on_start() override;`
28. [`DeviationScaleStrategy::on_trade_batch`](../Quant/include/strategy/DeviationScaleStrategy.h#L196) — 틱 → `aggregator_.on_tick` → 판단 직전 `close_stale`·`bars::resample` → 진입/청산 판단 → out. 매매 로직의 본체. 스탑·트레일 뒤 `stop_cooldown_sec`, 전량 청산 뒤 `reentry_cooldown_sec` 동안은 새 베이스를 깔지 않는다  
...
31. [`DeviationScaleStrategy::emit_liquidation`](../Quant/include/strategy/DeviationScaleStrategy.h#L331) — 청산 신호 조립 — 시장가면 `reference_price` 스탬프, 매도 가능 수량은 원장 접근자(`sellable_quantity`, 동기 잔고조회 금지)  
...
32. [`Engine::strategy_thread_fn`](../Quant/src/core/EngineStrategyThread.cpp#L189) — 루프 한 바퀴: `flush_held` → 강제청산(2초 재발주) → `trim_excess_once`(기동 20초 뒤 1회) → `pipeline_.shard_out` 비우기(`from_strategy`) → 잠(상한 10ms). 국면 파일은 데이터 스레드가 읽는다(6절)  
   `Quant/src/core/EngineStrategyThread.cpp:189` · `void Engine::strategy_thread_fn(std::stop_token stop_token)`
33. [`dispatch::SignalDispatcher::from_strategy`](../Quant/src/core/SignalDispatcher.cpp#L152) — 비활성 전략의 신규 매수·청산 관리(청산 관리) 종목의 신규 매수 차단 → `submit`. 판정 함수는 `set_exit_managed_check`으로 엔진이 준다  
...
35. [`dispatch::SignalDispatcher::force_liquidate`](../Quant/src/core/SignalDispatcher.cpp#L246) — 보유 전량 시장가 매도를 2초 간격 재발주. `reference_price`가 여기서 찍히는지 본다  
...
40. [`OrderRouter::submit`](../Quant/src/ipc/OrderRouter.cpp#L77) — `action`으로 분기만 — NEW는 `new_route`, CANCEL·REPLACE는 `cancel_route`·`replace_route`  
...
41. [`OrderRouter::new_route`](../Quant/src/ipc/OrderRouter.cpp#L89) — `clamp_buy_quantity` → 매도가능수량 0이면 `reconcile_blocked_sell` → 시장가 매도 중복 가드(`history_`) → `gate_.check` → `take_intent`(원장 INTENT 선기록·선점, 실패면 전송 안 함) → `kis_.submit_order_acknowledgement`(ODNO) → `gate_.on_accepted` → `record`·`write_trade_row`(`seq` 동반). 실패 경로마다 무엇이 되돌려지는지  
...
44. [`OrderGate::plan_displacement`](../Quant/src/risk/OrderGate.cpp#L808) — 슬롯이 찼을 때 어느 보유를 내보낼지. 디스패처의 교체 진입이 이 계획을 쓴다  
...
46. [`KisClient::submit_order_acknowledgement`](../Quant/src/api/KisOrder.cpp#L141) — 현금 주문 REST. tr_id(실/모의)·`EXCG_ID_DVSN_CD`(KRX/NXT/SOR, D-096)·`authentication_headers`·응답에서 ODNO. 여기서만 KIS에 주문이 닿는다  
...
48. [`KisWebSocket::parse_fill_notification`](../Quant/src/api/KisWebSocketParse.cpp#L378) — AES 복호화 → `decode_fill` → `on_fill_` 콜백(Engine이 `pipeline_.fill_queue.push`)  
   `Quant/src/api/KisWebSocketParse.cpp:378` · `void KisWebSocket::parse_fill_notification(kis_websocket::Fields fields)`
49. [`kis_websocket::decode_fill`](../Quant/include/api/KisWsDecode.h#L154) — 체결통보 필드 → `FillNotification`(ODNO·체결/거부·수량·가격). 거부 통보도 같은 채널  
...
51. [`OrderRouter::on_fill`](../Quant/src/ipc/OrderRouter.cpp#L2497) — ODNO로 주문 찾기 → 상태 갱신 → `gate_.ledger().on_fill_confirmed` → 원장 CSV. 못 찾으면 미연결 체결 경로  
...
52. [`PositionLedger::on_fill_confirmed`](../Quant/src/risk/PositionLedger.cpp#L827) — 포지션·평단·실현손익 갱신, `reserved_` 해제. `FillResult`가 실현 PnL을 돌려준다  
...
60. [`Engine::request_shutdown`](../Quant/src/core/Engine.cpp#L665) — 시그널 핸들러에서 불려도 되는 최소 동작(플래그·깨우기)만  
   `Quant/src/core/Engine.cpp:665` · `void Engine::request_shutdown(std::string_view reason, ipc::SharedShutdownReason recorded_reason)`
61. [`Engine::stop`](../Quant/src/core/Engine.cpp#L696) — stop_token 요청 → join 순서(control → order → 샤드 → strategy → data → WS 끊기 → fill 마지막, 큐를 비우고 끝난다) → ZMQ·운영단말 서버 정지 → 전략 `on_stop` → 통계 출력. 미체결 예약주문 기억은 여기서 사라진다(재기동 규칙, CLAUDE.md '장중 운영')  
   `Quant/src/core/Engine.cpp:696` · `void Engine::stop()`
...
63. [`feed::PaperExecutor`](../Quant/include/core/PaperExecutor.h#L35) — `IOrderExecutor` 모의 체결기 — 주문 즉시 체결통보를 만들어 같은 `pipeline_.fill_queue` 경로로 넣는다  
...
| [Quant/src/api/KisWebSocketParse.cpp](../Quant/src/api/KisWebSocketParse.cpp) | 12 `parse_message`, 13 `dispatch_record`, 14 `parse_kr_trade`, 48 `parse_fill_notification` |
| [Quant/src/api/WebSocketClient.cpp](../Quant/src/api/WebSocketClient.cpp) | 11 `recv_loop` |
...
| [Quant/src/core/Engine.cpp](../Quant/src/core/Engine.cpp) | 7 `start`, 9 `spawn_threads`, 60 `request_shutdown`, 61 `stop` |
| [Quant/src/core/EngineConfigure.cpp](../Quant/src/core/EngineConfigure.cpp) | 4 `configure` |
| [Quant/src/core/EngineControlThread.cpp](../Quant/src/core/EngineControlThread.cpp) | 53 `control_thread_fn`, 59 `activate_rest_fallback` |
| [Quant/src/core/EngineDataThread.cpp](../Quant/src/core/EngineDataThread.cpp) | 19 `data_thread_fn` |
| [Quant/src/core/EngineFeed.cpp](../Quant/src/core/EngineFeed.cpp) | 8 `connect_feed (WS 콜백 설치)` |
| [Quant/src/core/EngineFillThread.cpp](../Quant/src/core/EngineFillThread.cpp) | 50 `fill_thread_fn` |
| [Quant/src/core/EngineOpsServer.cpp](../Quant/src/core/EngineOpsServer.cpp) | 37 `take_manual_order` |
| [Quant/src/core/EngineOrderThread.cpp](../Quant/src/core/EngineOrderThread.cpp) | 36 `order_thread_fn` |
| [Quant/src/core/EngineRegime.cpp](../Quant/src/core/EngineRegime.cpp) | 54 `poll_regime_file`, 56 `apply_regime_selection` |
| [Quant/src/core/EngineStrategyThread.cpp](../Quant/src/core/EngineStrategyThread.cpp) | 6 `add_strategy`, 22 `shard_thread_fn`, 32 `strategy_thread_fn` |
| [Quant/src/core/EngineUniverse.cpp](../Quant/src/core/EngineUniverse.cpp) | 57 `maybe_rescan_universe` |
...
| [Quant/src/risk/OrderGate.cpp](../Quant/src/risk/OrderGate.cpp) | 42 `check`, 43 `clamp_buy_quantity`, 44 `plan_displacement` |
| [Quant/src/risk/PositionLedger.cpp](../Quant/src/risk/PositionLedger.cpp) | 45 `on_intent`, 52 `on_fill_confirmed` |
