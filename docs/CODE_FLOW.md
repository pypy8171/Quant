...
1. [`main`](../Quant/src/main.cpp#L318) — 진입. 번호 주석이 초기화 순서다 — 콘솔·로거 → 인자 → `parse_config` → 로그 임계값 → 크래시 핸들러 → 모드 분기(FEED·KR_TEST·US_TEST는 modes/, TRADE는 `run_trade`)  
...
6. [`Engine::add_strategy`](../Quant/src/core/Engine.cpp#L35) — 전략 등록. 심볼 해석기(`set_symbol_resolver` → `SymbolTable::intern`)가 여기서 주입된다  
   `Quant/src/core/Engine.cpp:35` · `void Engine::add_strategy(std::unique_ptr<StrategyBase> strategy)`
7. [`Engine::start`](../Quant/src/core/Engine.cpp#L1256) — 도우미 호출 목록이 기동 순서다 — `setup_shards`(행렬 `reshape`, 행=수신 스레드+폴러, 열=샤드) → ZMQ → 모의 체결기 → 라우터·대조기·폴러 → `try_bootstrap_ledger`(원장 시드) → `start_strategies` → 구독 목록 → `connect_feed` → `spawn_threads`  
   `Quant/src/core/Engine.cpp:1256` · `void Engine::start()`
8. [`Engine::connect_feed (WS 콜백 설치)`](../Quant/src/core/Engine.cpp#L1099) — 소켓 수신 스레드 i의 호가·체결 콜백. 종목 id로 열을 고르고(`consumer_of`) `pipeline_.trade_matrix`·`order_book_matrix`의 자기 행에 `push_to` — 가득 차면 버리고 센다(블로킹 금지)  
   `Quant/src/core/Engine.cpp:1099` · `feed_.websocket->set_lane_callbacks([this] (uint32_t lane, const OrderBook& in) …`
9. [`Engine::spawn_threads`](../Quant/src/core/Engine.cpp#L1243) — data·strategy·order·fill·control 다섯 jthread + 샤드 M. stop_token이 첫 인자라 람다로 감싼다  
   `Quant/src/core/Engine.cpp:1243` · `data_thread_     = std::jthread([this] (std::stop_token stop_token) { data_thread_fn(stop_token); });`
...
19. [`Engine::data_thread_fn`](../Quant/src/core/Engine.cpp#L1502) — REST 축 — 봉 폴링·`poll_regime_file`(국면)·잔고 대조·유니버스 재스캔·하루 경계(`new_trading_day`·`reset_daily`). 폴러의 대체 틱은 `pipeline_.trade_matrix`의 자기 행(`data_row`)으로 간다  
   `Quant/src/core/Engine.cpp:1502` · `void Engine::data_thread_fn(std::stop_token stop_token)`
...
21. [`feed::TickCapture::on_trade`](../Quant/include/core/TickCapture.h#L292) — raw 틱 append-only 캡처(원칙 8). 리플레이(`ReplaySource`)의 입력  
   `Quant/include/core/TickCapture.h:292` · `void on_trade(const TradeData& trade) noexcept` · 시험 [test_tick_capture](../Quant/tests/test_tick_capture.cpp)
...
22. [`Engine::shard_thread_fn`](../Quant/src/core/Engine.cpp#L2369) — 전략 집합 버전이 바뀌면 `rebuild`, 아니면 `step`. 비면 `WakeGate`로 잠든다. `emit`은 `pipeline_.shard_out` push + 디스패치 스레드 깨우기  
   `Quant/src/core/Engine.cpp:2369` · `void Engine::shard_thread_fn(std::stop_token stop_token, uint32_t row)`
...
26. [`StrategyBase::on_trade_batch`](../Quant/include/strategy/StrategyBase.h#L71) — 전략 훅의 계약(가상 함수 다섯). 기본 구현은 `on_trade` 하나를 out에 담는다. `symbol_of`·`same_symbol`도 이 헤더  
   `Quant/include/strategy/StrategyBase.h:71` · `virtual void on_trade_batch(const TradeData&, std::vector<OrderSignal>& /*out*/)`
...
28. [`DeviationScaleStrategy::on_trade_batch`](../Quant/include/strategy/DeviationScaleStrategy.h#L330) — 틱 → `aggregator_.on_tick` → 판단 직전 `close_stale`·`bars::resample` → 진입/청산 판단 → out. 매매 로직의 본체. 스탑·트레일 뒤 `stop_cooldown_sec`, 전량 청산 뒤 `reentry_cooldown_sec` 동안은 새 베이스를 깔지 않는다  
   `Quant/include/strategy/DeviationScaleStrategy.h:330` · `void on_trade_batch(const TradeData& trade, std::vector<OrderSignal>& out) override`
...
31. [`DeviationScaleStrategy::emit_liquidation`](../Quant/include/strategy/DeviationScaleStrategy.h#L1441) — 청산 신호 조립 — 시장가면 `reference_price` 스탬프, 매도 가능 수량은 원장 접근자(`sellable_quantity`, 동기 잔고조회 금지)  
   `Quant/include/strategy/DeviationScaleStrategy.h:1441` · `bool emit_liquidation(std::vector<OrderSignal>& out, int position, …`
...
32. [`Engine::strategy_thread_fn`](../Quant/src/core/Engine.cpp#L2259) — 루프 한 바퀴: `flush_held` → `drain_manual_inbox` → 강제청산(2초 재발주) → `trim_excess_once`(기동 20초 뒤 1회) → `pipeline_.shard_out` 비우기(`from_strategy`) → 잠(상한 10ms). 국면 파일은 데이터 스레드가 읽는다(6절)  
   `Quant/src/core/Engine.cpp:2259` · `void Engine::strategy_thread_fn(std::stop_token stop_token)`
...
36. [`Engine::drain_manual_inbox`](../Quant/src/core/Engine.cpp#L3080) — 운영단말 수동 주문(`ops_.manual_inbox`)이 같은 싱크로 들어온다 — 생산자를 늘리지 않기 위해 이 스레드가 꺼낸다  
   `Quant/src/core/Engine.cpp:3080` · `void Engine::drain_manual_inbox(const std::function<void(const OrderSignal&)>& emit)`
...
37. [`Engine::order_thread_fn`](../Quant/src/core/Engine.cpp#L2498) — `take_due_retry` 우선 → 큐 pop → `wait_before_send` → `router->submit` → 성공이면 `note_sent`, 거부면 `on_rejected` → `LatencyTrace::record`. 비면 재시도 만기까지 `wait_until`  
   `Quant/src/core/Engine.cpp:2498` · `void Engine::order_thread_fn(std::stop_token stop_token)`
...
40. [`OrderRouter::submit`](../Quant/src/ipc/OrderRouter.cpp#L64) — `action`으로 분기만 — NEW는 `new_route`, CANCEL·REPLACE는 `cancel_route`·`replace_route`  
   `Quant/src/ipc/OrderRouter.cpp:64` · `ManagedOrder OrderRouter::submit(const OrderSignal& signal)` · 시험 [test_order_router](../Quant/tests/test_order_router.cpp)
41. [`OrderRouter::new_route`](../Quant/src/ipc/OrderRouter.cpp#L76) — `clamp_buy_quantity` → 매도가능수량 0이면 `reconcile_blocked_sell` → 시장가 매도 중복 가드(`history_`) → `gate_.check` → `take_intent`(원장 INTENT 선기록·선점, 실패면 전송 안 함) → `kis_.submit_order_acknowledgement`(ODNO) → `gate_.on_accepted` → `record`·`write_trade_row`(`seq` 동반). 실패 경로마다 무엇이 되돌려지는지  
   `Quant/src/ipc/OrderRouter.cpp:76` · `ManagedOrder OrderRouter::new_route(const OrderSignal& in_signal)` · 시험 [test_order_router](../Quant/tests/test_order_router.cpp)
...
44. [`OrderGate::plan_displacement`](../Quant/src/risk/OrderGate.cpp#L1600) — 슬롯이 찼을 때 어느 보유를 내보낼지. 디스패처의 교체 진입이 이 계획을 쓴다  
   `Quant/src/risk/OrderGate.cpp:1600` · `OrderGate::DisplacePlan OrderGate::plan_displacement(const std::string& account, symbol::SymbolId new_symbol) const` · 시험 [test_order_gate](../Quant/tests/test_order_gate.cpp)
45. [`OrderGate::on_intent`](../Quant/src/risk/OrderGate.cpp#L715) — 전송 **전에** 원장 저널에 INTENT를 적고 `reserved_`를 선점한다(슬롯·현금). 기록에 실패하면 선점을 되돌리고 거짓을 준다 — 그 주문은 나가지 않는다(D-113). 접수 뒤 짝은 `on_accepted`, 되돌리는 짝은 `on_fill_confirmed`·`on_cancel`·`on_reject`  
   `Quant/src/risk/OrderGate.cpp:715` · `bool OrderGate::on_intent(const std::string& account, const std::string& ticker, OrderSide side, int quantity, …` · 시험 [test_order_gate](../Quant/tests/test_order_gate.cpp)
...
47. [`trace::LatencyTrace::record`](../Quant/include/core/LatencyTrace.h#L189) — 틱 수신→신호→pop→라우터 반환 네 시각을 `logs/latency_trace.csv` 한 줄로  
   `Quant/include/core/LatencyTrace.h:189` · `void record(const OrderSignal& signal, const Marks& marks, bool kis_called, bool accepted)` · 시험 [test_latency_trace](../Quant/tests/test_latency_trace.cpp)
...
50. [`Engine::fill_thread_fn`](../Quant/src/core/Engine.cpp#L2606) — `pipeline_.fill_queue` pop → `on_fill` → `ledger_->note_fill`(대조 5초 유예, D-074) → 운영단말 `broadcast`(FILL). 비면 `WakeGate`  
   `Quant/src/core/Engine.cpp:2606` · `void Engine::fill_thread_fn(std::stop_token stop_token)`
51. [`OrderRouter::on_fill`](../Quant/src/ipc/OrderRouter.cpp#L1867) — ODNO로 주문 찾기 → 상태 갱신 → `gate_.on_fill_confirmed` → 원장 CSV. 못 찾으면 미연결 체결 경로  
   `Quant/src/ipc/OrderRouter.cpp:1867` · `void OrderRouter::on_fill(const FillNotification& fill_notification)` · 시험 [test_order_router](../Quant/tests/test_order_router.cpp)
52. [`OrderGate::on_fill_confirmed`](../Quant/src/risk/OrderGate.cpp#L1355) — 포지션·평단·실현손익 갱신, `reserved_` 해제. `FillResult`가 실현 PnL을 돌려준다  
   `Quant/src/risk/OrderGate.cpp:1355` · `OrderGate::FillResult OrderGate::on_fill_confirmed( …` · 시험 [test_order_gate](../Quant/tests/test_order_gate.cpp)
...
53. [`Engine::control_thread_fn`](../Quant/src/core/Engine.cpp#L2761) — 큐 고수위 1분 로그 → 토큰 선갱신(5분, 만료 30분 전) → 손익 갱신 감시(끊기면 `set_kill_switch` 보수정지) → WS stale·재연결·REST 폴백(`feed::Supervisor` 판정)  
   `Quant/src/core/Engine.cpp:2761` · `void Engine::control_thread_fn(std::stop_token stop_token)`
54. [`Engine::poll_regime_file`](../Quant/src/core/Engine.cpp#L2155) — 데이터 스레드가 부른다. `regime.json` 축 — `entry_halt`(신규매수 차단)·`entry_scale`(매수비율)·`force_liquidate`, 그리고 라벨 전이 때 `apply_regime_selection`(전략 집합 선택, D-084). 상태기계는 `RegimeFileJudge.h`  
   `Quant/src/core/Engine.cpp:2155` · `void Engine::poll_regime_file()` · 시험 [test_regime_file_judge](../Quant/tests/test_regime_file_judge.cpp)
55. [`OrderGate::set_manual_halt`](../Quant/include/risk/OrderGate.h#L350) — 운영단말 HALT_REQ의 수동 정지 — 신규 매수·전략 매도를 따로 끈다. 국면의 `entry_halt_`와는 다른 플래그고 `is_entry_halted`에서만 OR로 합친다(D-091)  
   `Quant/include/risk/OrderGate.h:350` · `void set_manual_halt(OrderSide side, bool on)` · 시험 [test_order_gate](../Quant/tests/test_order_gate.cpp)
56. [`Engine::apply_regime_selection`](../Quant/src/core/Engine.cpp#L157) — 국면 → `regime_strategies` 집합으로 전략 활성/비활성. 청산은 하지 않는다  
   `Quant/src/core/Engine.cpp:157` · `void Engine::apply_regime_selection(Regime regime, bool force_log)`
57. [`Engine::maybe_rescan_universe`](../Quant/src/core/Engine.cpp#L364) — 유니버스 재스캔 — 빠진 보유 종목은 40초에 신규매수 차단, 600초에 전략 해제(`UniverseExit.h`, D-077). 청산 관리 보유(`exit_managed_tickers`)는 스캔 신규매수에서 뺀다  
   `Quant/src/core/Engine.cpp:364` · `void Engine::maybe_rescan_universe()` · 시험 [test_signal_dispatcher](../Quant/tests/test_signal_dispatcher.cpp)
...
59. [`Engine::activate_rest_fallback`](../Quant/src/core/Engine.cpp#L2723) — WS가 stale이면 REST 현재가 폴링으로 대체 틱(`received_ns`=0). 복귀는 `deactivate_rest_fallback`  
   `Quant/src/core/Engine.cpp:2723` · `bool Engine::activate_rest_fallback(const std::string& reason)`
...
60. [`Engine::request_shutdown`](../Quant/src/core/Engine.cpp#L1405) — 시그널 핸들러에서 불려도 되는 최소 동작(플래그·깨우기)만  
   `Quant/src/core/Engine.cpp:1405` · `void Engine::request_shutdown(std::string_view reason)`
61. [`Engine::stop`](../Quant/src/core/Engine.cpp#L1424) — stop_token 요청 → join 순서(control → order → 샤드 → strategy → data → WS 끊기 → fill 마지막, 큐를 비우고 끝난다) → ZMQ·운영단말 서버 정지 → 전략 `on_stop` → 통계 출력. 미체결 예약주문 기억은 여기서 사라진다(재기동 규칙, CLAUDE.md '장중 운영')  
   `Quant/src/core/Engine.cpp:1424` · `void Engine::stop()`
