...
51. [`OrderRouter::on_fill`](../Quant/src/ipc/OrderRouter.cpp#L2099) — ODNO로 주문 찾기 → 상태 갱신 → `gate_.on_fill_confirmed` → 원장 CSV. 못 찾으면 미연결 체결 경로  
   `Quant/src/ipc/OrderRouter.cpp:2099` · `void OrderRouter::on_fill(const FillNotification& fill_notification)` · 시험 [test_order_router](../Quant/tests/test_order_router.cpp)
