...
| `Quant/include/**/*.h`, `Quant/src/**/*.cpp`, `scripts/*.py`, `scripts/*.ps1`, `PYQuant/tools/*.py`, `PYQuant/data/*.py`, `docs/tuning_sheet.toml`, `scripts/gen_tuning_sheet.py` | `_private/TUNING_SHEET.md` | 자동(명령) `py scripts/gen_tuning_sheet.py --check` | 장중 매매 수치·주기 시트는 재생성(fix_cmd). config 변경은 git diff 에 안 잡히므로 sync-gate 가 매 턴 --check 를 따로 돈다. 앵커 정규식이 안 잡히면 tuning_sheet.toml 을 고친다 |
