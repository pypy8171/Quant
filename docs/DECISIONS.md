...
- `Quant/tests/test_engine.cpp`: 가짜 소스가 콜백을 받아 두고 테스트 스레드가 레인 0으로 틱을 넣는다. 첫 틱에 시장가
...
**Phase 4 앞 단계 다섯째 조각 — 수신 N×샤드 M을 한 바퀴로 고정한다 (2026-09-14)** — 배선(레인마다 행렬 행, 샤드마다 열,
`strategy_shards`·`feed_keys`)은 앞 네 조각에서 다 들어갔는데 실제로 N>1·M>1을 같이 돌려 본 시험이 없었다 — `test_engine`은
1×1만 봤고 조각 시험들은 행렬·샤드를 따로 봤다. `test_engine`을 케이스 함수로 바꿔 레인 1×샤드 1에 이어 레인 2×샤드 2를 돈다:
종목 둘(`005930`·`000660`)이 각자 다른 레인(테스트 스레드가 레인 번호를 달아 콜백)으로 들어오고, 두 종목의 id가 서로 다른
열에 해시되는지(`shard::shard_of`)를 먼저 확인한 뒤 종목마다 주문 접수 1건·다음 틱 체결·보유 1주·평단을 본다. 기동 뒤 실제로
잡힌 레인·샤드 수는 새 접근자 `Engine::ws_lanes()`·`shard_count()`로 읽는다 — config가 2여도 걸치는 전략이 있으면 1로 돌아가므로
config가 아니라 실제 값을 검사한다. 34 checks, 두 케이스 합쳐 0.1초 안. 시험용 소스 `lanes()`가 N을 답하면 Engine이 행을 N개 잡는
자리는 FeedMux 레인 모드와 같다. 다음: Phase 4 본단계(코어·부하로 정하는 수신 스레드 수, 레인별 stale·재연결).
...
- 취소 전용 레인(D-081 ④)은 만들지 않는다 — KIS 초당 한도는 취소에도 걸리므로 레인을 나눠도 처리량이 늘지 않는다.
