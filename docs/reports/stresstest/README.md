...
| 2026-09-25 | [프로세스 셋으로 나눈 뒤 — 정합성·부하 (OVERVIEW 6절)](OVERVIEW.md#6-09-25--프로세스-셋으로-나눈-뒤) | `rounds_three.py` + `kill_round.py` + `config_load_test.json`·`config_equivalence.json` | 같은 입력을 1 프로세스와 3 프로세스(시세·전략·주문)로. 300종목 × 1,000건(DB 끔)으로 정합성, 2,700종목 × 30,000건(DB 켬)으로 지연, 그리고 셋 중 하나를 죽여 본다 | 정합성 **통과**(갈라진 종목 0·빠진 순번 0). 분리로 늘어난 구간은 **시세→전략 경계 하나뿐**이고 주문 스레드 안쪽은 `꺼냄→반환`이 369 → **35µs**(10.5배) — 큰 판 전 구간 p50이 361,630 → **97,682µs**로 오히려 73% 줄었다. 가는 길에 **천장 셋**(전략 이름표 1024·시세 제어 줄 1024·주문 스레드 유휴 100ms)을 만나 고쳤다(D-136) |
