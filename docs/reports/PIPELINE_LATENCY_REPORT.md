...
찍는 후속이 없앨 값이다. 09-20 D-106이 이 조회의 읽기 락을 뗐다 — 단일 스레드 27.5 → 24.9ns, 수신 스레드 4개 동시는
스레드당 110~178 → 18~20ns(같은 벤치 2절 마지막 행).
...
## 결과 ⑦ — 함수별 CPU 비중 (perf 핫스팟, 2026-09-22)

지금까지 이 리포트에는 "얼마나 걸리나"만 있었다. "어느 함수가 CPU를 먹나"는 없었다. 윈도우에 붙일 도구가 없어서인데,
WSL 리눅스 빌드에 perf를 붙이면 받을 수 있다(절차는 가이드 5절).

`bench_market_firehose load --tickers 2600 --rate 200000 --duration 15`, 코어 8~15에 묶음, cpu-clock 499Hz, 표본 21,000건.
드롭 0, 실측 199,987 msg/sec.

| 비중 | 바이너리 | 함수 |
|---|---|---|
| 33.73% | bench_market_firehose | `order_fn` — 주문단 소비 루프 |
| 33.28% | bench_market_firehose | `strategy_fn` — 전략단 소비 루프 |
| 27.31% | [vdso] | `__vdso_clock_gettime` |
| 2.34% | libstdc++ | `std::chrono::steady_clock::now()` |
| 1.86% | bench_market_firehose | `producer_fn` — 생산 루프 |

**읽는 법.** 소비 루프 둘의 30%대는 일한 시간이 아니라 busy-wait으로 기다린 시간이다(이 하네스는 sleep을 안 쓴다).
병목이 아니다. 실제로 일한 시간 쪽에서는 시계 읽기가 약 30%로 압도적인데, 이 하네스가 지연을 재려고 메시지마다
타임스탬프를 찍기 때문에 그중 상당 부분이 측정 비용이다. 그대로 라이브 엔진 수치로 옮기면 안 된다.

**남는 질문 — 라이브 엔진도 같은 자리에서 시계를 읽는가.** [Engine.cpp](../../Quant/src/core/Engine.cpp)의 디스패처 루프는
반복마다 `steady_clock::now()`를 한 번 읽고, 그 값으로 `flush_held`·`drain_manual_inbox`·`trim_excess_once` 세 가지 시각
기반 정리를 매번 부른다. 일이 있으면 곧장 `continue`하므로 반복 횟수는 "큐가 비는 횟수"인데, 소비자가 생산자를 따라잡는
구간에서는 그게 사실상 메시지당 한 번이 된다. 2,500종목 목표 부하(D-071)에서는 이게 그대로 비용으로 남는다.
**이건 코드에서 읽은 가설이고 실측이 아니다.** 확인하려면 반복 횟수와 처리 메시지 수를 같이 세는 회차가 필요하다.

**아직 못 받는 것.** 진짜 Engine 코드의 부하 상태 핫스팟. 이 하네스는 Engine.cpp와 같은 3단 토폴로지를 재현한 것이지
Engine 자체가 아니다. 라이브 트레이더는 CPU 1~2%대라 perf 문턱(5%)을 안 넘어서 뜨지 않는다. `bench_engine_load`(전 종목
부하 하네스)를 리눅스로 빌드하면 받을 수 있는데, 그 하네스의 `--no-orders` 구성은 전략이 지표 계산 없이 카운터만 올리는 구성이라
상위 함수가 큐·전송 쪽으로 쏠린다 — 파이프라인 병목 위치로는 쓸 만하고, 전략 계산 비중으로는 못 읽는다.
