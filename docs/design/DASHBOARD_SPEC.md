...
→ 따라서 **정규화 계약(스키마)을 먼저 박고**, 대시보드는 그 계약만 읽는다.
...
- **렌더링 스택 = FastAPI + 경량 HTML/JS 단일 홈.** 라이브는 어차피 서버 프로세스가 필요하므로 그 프로세스가 백테스트 뷰도 서빙 → 하네스와 같은 Python으로 스택 수렴. 백테스트 뷰는 **정적 HTML로도 export**해 서버 없이 열람·공유 가능.
...
**정직성 필드가 왜 필수인가** — 대시보드가 예뻐지면 없는 신뢰를 만든다. BT-09는 홀드아웃 전패·정직한 실패가 다수인데(`research/studies/09_...`), 지표 카드만 크게 띄우면 그 맥락이 지워진다. `honesty_label`·`oos_flag`·`holdout_flag`를 스키마에 못 박아 카드가 맥락과 함께 표시되게 강제한다. → [GUARDRAILS](../../research/GUARDRAILS.md), `bias-auditor`.
...
- [ ] 백테스트 HTML 대시보드 — 계열 A(`*_metrics.json` 단일객체) + 계열 B(`metrics.json` 배열) + equity CSV를 읽어 `family`별 지표표/곡선/정직성 라벨 렌더(+정적 export).
