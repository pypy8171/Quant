...
| **함정** | `PYQuant/bt_*.json`은 확장자만 json, **실내용은 CSV** | `regime.json`은 엔진 안 국면 스레드(`Quant/src/regime/RegimeFeed.cpp`)가 쓰고 같은 엔진이 다시 읽는 **파일**이다 — 대시보드도 이 파일을 읽는다(ZMQ로 오지 않는다, D-147). ZMQ 브리지는 vcpkg ZeroMQ로 **켜져 있고** 체결 기록기(`PYQuant/main.py record`)가 TimescaleDB에 적재한다(D-090) |
...
**정직성 필드가 필수인 이유**: 대시보드가 보기 좋아지면 근거 없는 신뢰가 생긴다. 위기 대응·수익추구 전략 10종은 홀드아웃(격리검증) 전패·기대 우위 없음이 다수인데(`research/studies/09_...`), 지표 카드만 크게 띄우면 그 맥락이 지워진다. `honesty_label`·`oos_flag`·`holdout_flag`를 스키마에 못 박아 카드가 맥락과 함께 표시되게 강제한다. → [GUARDRAILS](../../research/GUARDRAILS.md), `bias-auditor`.
...
**계열 B 백필 규약** — 위기 연구는 종목 포트폴리오가 아니라 **지수 익스포저 0~1.2x 오버레이**라 계열 A와 직접 비교 불가. `family="B_overlay"`+`benchmark`로 대시보드가 비교군을 분리한다. 오버레이엔 무의미한 `win_rate`/`n_trades`는 `null`. `mdd`는 계열 A(양수)와 통일해 **양수 크기로 정규화**(연구 스크립트의 `curve_stats`는 음수 mdd → `abs()`). `alpha`=전략 연복리(CAGR)−매수 후 보유 CAGR(%p). 위기 대응·수익추구 전략 10종은 홀드아웃(2022) 맥락을 `holdout_calmar`/`train_calmar` extra로 보존(예: C1 train +0.18 → holdout −0.71, 전패 사실이 카드에서 지워지지 않게). 전 행 `honesty_label="robust"`(look-ahead 삼중차단·홀드아웃 잠금으로 **방법론**은 견고, 승패는 지표값이 말함).

- **위기 레짐 지수레벨 특성화(07_crisis_regimes)는 제외** — 위기 국면 *특성화*(peak/trough/max-dd/회복일수/shape)이지 전략 성과가 아니라 `quant.metrics/v1` 스키마에 맞지 않는다. 대시보드에선 위기 배경 참조 데이터셋으로 별도 취급(성과표엔 넣지 않음).
...
- [x] 계열 B 백필 — 08·09 스크립트에 `_emit_metrics` 훅 추가, 실행 시 `metrics.json` 배열(42행) 산출. `family=B_overlay`, 최대낙폭(MDD) 양수정규화, 홀드아웃 맥락 보존. **위기 레짐 지수레벨 특성화 제외**(특성화 데이터, 성과 아님).
