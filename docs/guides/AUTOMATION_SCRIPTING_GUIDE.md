...
- **크래시 루프**: 최근 30분 안 세션 종료 3회면 `exit 3`(정본 `scripts/auto_trade_day.ps1`의 `$crashWindow`·`$crashMaxExits`; 아래 연습 코드의 축소판 값과 다르다). 원인이 배선(config·인증·바이너리)에 있으면 재기동으로 안 풀리고 계좌만 반복 호출한다.
