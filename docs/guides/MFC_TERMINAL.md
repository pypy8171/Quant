...
<!-- sync: Quant/tools/ops_terminal/OpsTerminalDlg.cpp@377ec12 Quant/tools/ops_terminal/OpsTerminalDlg.h@6df8dba Quant/tools/ops_terminal/OpsLink.cpp@2346dca Quant/tools/ops_terminal/OpsLink.h@6aba3c3 Quant/include/ipc/OpsProtocol.h@c1c4b7e -->
...
| 2026-09-11 | 현재가가 없어 팔 자리를 볼 수 없음 → 엔진에 종목별 최근 체결가 캐시(지금 이름 `Engine::SymbolCache::last_price_array`, 전략 스레드가 틱마다 씀)를 두고 POSITIONS에 `last` 필드 추가. 60초 넘게 틱이 없는 보유 종목(유니버스 밖, WS 구독 상한에 밀린 종목)은 데이터 스레드가 매 사이클(30초) REST 현재가로 보충한다. 표에 현재가·평단대비 열, 폼에 현재가 줄(`IDC_CUR_PRICE`), `ref_price` stamp. 갱신마다 스크롤이 맨 위로 가던 것을 행 단위 갱신으로 고침 |
...
| 2026-09-18 | 전략 매도 정지 버튼(`IDC_HALT_SELL`) 추가(D-095). `HALT_REQ`에 `side`, `STATUS`·`HALT_ACK`의 수동 정지 필드 하나가 `manual_buy_halt`·`manual_sell_halt` 둘로. 매수 정지 버튼 라벨을 "신규 매수 정지"로 |
