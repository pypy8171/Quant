...
CV_SMALL = "소표본 N=10~89: 게이트(≥200) 미달 — 통계 아닌 일화. 지표 4종 전기간 재검증으로만 판정."
...
# ── 모멘텀·국면필터 롤링검증(1~5년) · 모멘텀×국면필터 롤링 (BACKTEST_LOG 실행 #1) ──────────────────────────
BT01 = [
...
# ── 2022 약세장 국면필터 제거실험 · 2022 약세장 표본외(OOS) + 국면 ON/OFF 제거실험 (실행 #3) ─────────────────────
BT03 = [
...
# ── 월별 시작시점 스윕 · 2026 월별 시작 민감도 스윕 (실행 #4, 소표본) ─────────────────────────
...
# ── 지표 4종 전기간 검증 · 전기간 지표4종 재검증 2021~2026, N≥750 (실행 #5) ─────────────────────
...
# ── 하락장 유사구간 6구간 비교 · 하락장 이벤트 백테스트 (summary_*.tsv 파싱) ──────────────────────────
