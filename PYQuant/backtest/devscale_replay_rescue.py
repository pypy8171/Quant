#!/usr/bin/env python3
"""devscale_replay.py의 변형본 — 개장 이격 조건의 **재평가 시점**만 바꿔 본다(스터디 25).

원본과 다른 곳은 세 군데뿐이다.
  1) Params에 `entry_rescue_hhmm` 추가 — >0이면 그 시각에 구제 판정을 한 번 한다.
  2) replay_day에서 하루 진입 필터를 ATR 조건(atr_ok)과 이격 조건(deviation_ok)으로 쪼갰다.
     ATR 조건은 전일 확정 일봉에서 나온 값이라 장중 재평가 대상이 아니다(막히면 그날은 끝).
  3) 이격 조건만 못 넘긴 날, 구제 시각의 **닫힌 봉 종가**(= 그 시각 현재가)로 같은 이격식을 다시 재
     통과하면 그 봉부터 진입을 연다. 이미 통과한 날은 다시 닫지 않는다(구제는 한 방향).
     주문은 그 다음 봉에서 체결 판정되므로 룩어헤드가 아니다.

  재현: py -X utf8 PYQuant/backtest/devscale_replay_rescue.py --pairs C:/build_tmp/minute_pairs_100.json \\
            --set rescue --jobs 6 --out research/studies/25_devscale_open_dev_rescue/rescue_grid.tsv
  커밋 afc9a3f · 데이터 PYQuant/data/minute(2025-09-19~2026-09-18, 296종목×243일) · 난수 없음(결정론).

원본 머리말(그대로 둔다) — DeviationScale TRENDX 슬리브의 3분봉 리플레이.
...
표로 있고, 여기서는 코드 옆에 짧게만 적는다.
  ...
  - 워밍업: 3분봉 < simple_moving_average_period(20)이면 기준선 = 일봉 SMA20, BUY 분할 단계 차단(베이스 BUY는 허용, 엔진과 같다).
...
def round_tick(price: float, side: str) -> float:
    tick = tick_size(price)
    return math.floor(price / tick) * tick if side == "BUY" else math.ceil(price / tick) * tick


@dataclass
class Params:
    entry_lower_percent: float = 5.0
    entry_upper_percent: float = 35.0
    zone_hysteresis_percent: float = 4.0
    align_tolerance_percent: float = 0.0
    simple_moving_average_period: int = 20
    deviation_sell_percent: float = 3.0
    deviation_buy_percent: float = 1.0
    split_step_count: int = 1
    buy_split_steps: int = 1
    notional_krw: float = 2_500_000.0
    base_percent: float = 0.015
    max_percent: float = 0.022
    ...
    trail_simple_moving_average_exit: bool = False
    trail_simple_moving_average_tolerance_percent: float = 1.0
    market_close_hhmm: int = 1515
    interval_min: int = 3
    max_notional_per_ticker: float = 15_000_000.0   # OrderGate risk.max_notional_per_ticker — 전략 밖 상한
    # ── 눌림 슬리브(라이브 DEVSCALE, 09-21) — entry_lower_percent=0이면 존 하단은 SMA20 아래 −pullback_percent ──
    ...
    entry_rescue_hhmm: int = 0           # >0이면 이 시각에 개장 이격 조건만 한 번 더 판정한다(ATR 조건은 그대로)


VARIANTS = {
    "v1_current":         Params(buy_split_steps=1),
    "v2_norung":          Params(buy_split_steps=0),
    "v3_norung_stop2.5":  Params(buy_split_steps=0, stop_loss_percent=2.5),
    "v4_norung_stop_trail": Params(buy_split_steps=0, stop_loss_percent=2.5, trail_simple_moving_average_exit=True),
}
...
EXEC_VARIANTS = {
    "e0_base_stop2.5":  Params(buy_split_steps=0, stop_loss_percent=2.5),
    ...
    "e6_delay1":        Params(buy_split_steps=0, stop_loss_percent=2.5, entry_confirm_bars=1),
    "e7_delay2":        Params(buy_split_steps=0, stop_loss_percent=2.5, entry_confirm_bars=2),
    "e8_delay3":        Params(buy_split_steps=0, stop_loss_percent=2.5, entry_confirm_bars=3),
...
_LIVE = Params(pullback_percent=8.0, entry_upper_percent=5.0, entry_lower_percent=0.0, zone_hysteresis_percent=4.0, simple_moving_average_period=20,
               base_on_price=False, buy_split_steps=0, notional_krw=4_350_000.0, base_percent=0.032, max_percent=0.064,
               reentry_cooldown_bars=4, stop_cooldown_bars=120,
               deviation_sell_percent=1.2, sell_base_average=True, split_step_count=1, stop_loss_percent=2.0)


def _live_grid() -> dict[str, Params]:
    """09-21 라이브(l0)를 기준으로 손절·익절·트레일을 한 번에 하나씩 바꾼 격자 + 09-20 전 설정(old)."""
    out = {
        "old_09-04":  replace(_LIVE, sell_base_average=False, deviation_sell_percent=1.5, split_step_count=2, stop_loss_percent=0.0),
        "l0_live":    _LIVE,
    }
    for stop in (0.0, 1.5, 2.5, 3.0):
        out[f"stop{stop:.1f}"] = replace(_LIVE, stop_loss_percent=stop)
    for take in (0.8, 1.0, 1.5, 2.0, 3.0):
        out[f"take{take:.1f}"] = replace(_LIVE, deviation_sell_percent=take)
    for arm, trail in ((1.0, 1.0), (1.0, 0.7), (1.5, 1.0), (0.7, 0.7), (2.0, 1.0)):
        # 트레일은 지정가 익절과 같이 둔다(먼저 닿는 쪽). 익절선을 트레일 무장선 위로 올려 트레일이 주된 청산이 되게 한다.
        out[f"trail_arm{arm:.1f}_t{trail:.1f}"] = replace(_LIVE, trail_arm_percent=arm, trail_percent=trail, deviation_sell_percent=3.0)
    out["trail_arm1.0_t1.0_take1.2"] = replace(_LIVE, trail_arm_percent=1.0, trail_percent=1.0)
    out["trail_arm1.0_t1.0_take2.0"] = replace(_LIVE, trail_arm_percent=1.0, trail_percent=1.0, deviation_sell_percent=2.0)
    return out


def _entry_grid() -> dict[str, Params]:
    """진입 시각 격자 — 라이브(l0)와 트레일(arm1.0/t1.0, 익절 3.0)에 개장 뒤 대기를 붙인다."""
    trail = replace(_LIVE, trail_arm_percent=1.0, trail_percent=1.0, deviation_sell_percent=3.0)
    out = {}
...
def _carry_grid() -> dict[str, Params]:
    ...
    for take in (1.2, 2.0, 3.0, 5.0):
        for stop in (1.5, 2.0, 3.0):
            out[f"carry_take{take}_stop{stop}"] = replace(_LIVE, carry_overnight=True, deviation_sell_percent=take, stop_loss_percent=stop)
    for arm, trail in ((1.0, 1.0), (1.5, 1.5), (2.0, 2.0), (3.0, 2.0)):
        out[f"carry_trail{arm}_t{trail}_take5"] = replace(_LIVE, carry_overnight=True, deviation_sell_percent=5.0, stop_loss_percent=2.0,
                                                          trail_arm_percent=arm, trail_percent=trail)
    for days in (2, 3, 5):
        out[f"carry_take3.0_stop2.0_max{days}d"] = replace(_LIVE, carry_overnight=True, deviation_sell_percent=3.0, stop_loss_percent=2.0,
                                                          carry_max_days=days)
    return out
    ...
    for take in (1.2,):
        for stop in (3.0, 5.0):
            for fname, filter_values in filters.items():
                out[f"c_take{take}_stop{stop}_{fname}"] = replace(_LIVE, carry_overnight=True, deviation_sell_percent=take, stop_loss_percent=stop, **filter_values)
    return out
...
_CARRY_BEST = replace(_LIVE, carry_overnight=True, deviation_sell_percent=1.2, stop_loss_percent=3.0,
            ...
            for atr in (4.0, 4.5, 5.0):
                for deviation in (-3.0, -1.0, 0.0):
                    out[f"w_take{take}_stop{stop}_atr{atr}_dev{deviation}"] = replace(
                        _LIVE, carry_overnight=True, deviation_sell_percent=take, stop_loss_percent=stop,
    ...
    for stop in (5.5, 6.0, 6.5, 7.0):
        for atr in (4.0, 4.5, 5.0, 5.5, 6.0):
            out[f"f_take3.0_stop{stop}_atr{atr}_dev-1.0"] = replace(
                _LIVE, carry_overnight=True, deviation_sell_percent=3.0, stop_loss_percent=stop,
                entry_atr_max_percent=atr, entry_open_deviation_min_percent=-1.0)
    return out
...
# D-111 채택값 — 익절 평단 +3.0%·손절 −6.5%·ATR14/SMA20 ≤4%·개장 이격 ≥−1%·넘김(장 마감 청산 없음).
_ADOPTED = replace(_LIVE, carry_overnight=True, deviation_sell_percent=3.0, stop_loss_percent=6.5,
                   entry_atr_max_percent=4.0, entry_open_deviation_min_percent=-1.0)


def _rescue_grid() -> dict[str, Params]:
    """개장 이격 조건의 재평가 시점만 바꾼다. 다른 파라미터는 채택값 고정, ATR 조건은 손대지 않는다.

    base_0903 = 현재 코드(09:03 단일 판정). rescue_HHMM = 09:03에 이격을 못 넘긴 날 그 시각에 한 번 더.
    10:30이 과제 대상이고 나머지 시각은 이웃 칸(시각 하나에 뾰족한 값인지 보는 용도).
    """
    out = {"base_0903": _ADOPTED}
    for hhmm in (1000, 1030, 1100, 1200, 1330):
        out[f"rescue_{hhmm:04d}"] = replace(_ADOPTED, entry_rescue_hhmm=hhmm)
    # 위 칸의 폭을 재는 기준선 — 개장 이격 조건만 아예 끈다(ATR≤4는 그대로). 구제의 최대치가 여기다.
    out["dev_filter_off"] = replace(_ADOPTED, entry_open_deviation_min_percent=-99.0)
    return out


RESCUE_VARIANTS = _rescue_grid()


LIVE_VARIANTS = _live_grid()


def resample(df1: pd.DataFrame, interval_minutes: int) -> pd.DataFrame:
    """1분봉 → interval_minutes분봉. scripts/dashboard_server.py::_resample과 같은 절대 분 버킷(09:00·09:03…)."""
    mins = df1["time"].str[:2].astype(int) * 60 + df1["time"].str[2:4].astype(int)
    key = mins // interval_minutes
    grouped = df1.groupby(key, sort=True)
    out = grouped.agg(open=("open", "first"), high=("high", "max"), low=("low", "min"),
                close=("close", "last"), volume=("volume", "sum"))
    out["hhmm"] = [(bucket_index * interval_minutes) // 60 * 100 + (bucket_index * interval_minutes) % 60
                   for bucket_index in out.index]
...
class DailyBook:
    ...
    def __init__(self, tickers: set[str]):
        daily_bars = pd.read_parquet(DAILY_PARQUET, columns=["Date", "code", "High", "Low", "Close"])
        daily_bars = daily_bars[daily_bars["code"].isin(tickers)]
        self.bars: dict[str, pd.DataFrame] = {}
        for code, ticker_group in daily_bars.groupby("code"):
            ticker_bars = ticker_group.set_index("Date")[["High", "Low", "Close"]].astype(float)
            ticker_bars.index = ticker_bars.index.strftime("%Y%m%d")
            self.bars[code] = ticker_bars
        self._extended: set[str] = set()

    def _extend_from_minute(self, ticker: str) -> None:
        if ticker in self._extended:
            return
        self._extended.add(ticker)
        existing_bars = self.bars.get(ticker)
        if existing_bars is None:
            existing_bars = pd.DataFrame(columns=["High", "Low", "Close"], dtype=float)
        last = existing_bars.index.max() if len(existing_bars) else "00000000"
        extra = {}
        for parquet_path in sorted((MINUTE_DIR / ticker).glob("*.parquet")):
            day = parquet_path.stem
            if day <= last:
                continue
            minute_bars = pd.read_parquet(parquet_path, columns=["high", "low", "close"])
            if len(minute_bars) >= 300:
                extra[day] = (float(minute_bars["high"].max()), float(minute_bars["low"].min()), float(minute_bars["close"].iloc[-1]))
        if extra:
            add = pd.DataFrame.from_dict(extra, orient="index", columns=["High", "Low", "Close"])
            self.bars[ticker] = pd.concat([existing_bars, add]).sort_index()
...
def aligned(simple_moving_averages: dict, tolerance_percent: float) -> bool:
    return (simple_moving_averages[5] > simple_moving_averages[10]
            and simple_moving_averages[10] > simple_moving_averages[20]
            and simple_moving_averages[20] > simple_moving_averages[60] * (1.0 - tolerance_percent / 100.0))
...
def replay_day(bars: pd.DataFrame, moving_averages: dict, parameters: Params, cost: CostSpec, atr14: float = 0.0,
    ...
    """
    al = aligned(moving_averages, parameters.align_tolerance_percent)
    average_20 = moving_averages[20]
    base_share = parameters.base_percent / parameters.max_percent if parameters.max_percent > parameters.base_percent else 1.0
    ...
    atr_ok = parameters.entry_atr_max_percent <= 0.0 or atr_percent <= parameters.entry_atr_max_percent
    entry_allowed = (atr_ok and parameters.entry_open_deviation_min_percent
                     <= open_deviation_percent <= parameters.entry_open_deviation_max_percent)
    rescue_done = False                # 구제 판정은 하루 한 번
    rescued = False                    # 이 날 구제로 진입이 열렸는가(집계용)
    exits: dict[str, int] = {}
    closes: list[float] = []
    bar_count = len(bars)
    done = False
    ...
    ohlc_array = bars[["open", "high", "low", "close", "hhmm"]].to_numpy(dtype=float)   # 봉당 .iloc 회피(같은 값)
    current_bar = 0
    for bar_index in range(bar_count):
        current_bar = bar_index
        bar_open, bar_high, bar_low, bar_close = (float(ohlc_array[bar_index, 0]), float(ohlc_array[bar_index, 1]),
                                                   float(ohlc_array[bar_index, 2]), float(ohlc_array[bar_index, 3]))
        hhmm = int(ohlc_array[bar_index, 4])
        # 1) 직전 봉에서 낸 주문을 이 봉 범위로 체결 판정한다. 하락봉이면 BUY부터, 상승봉이면 SELL부터.
        up = bar_close >= bar_open
        for kind, price, quantity, tag in sorted(orders, key=lambda x: (x[0] != ("SELL" if up else "BUY"))):
            if kind == "MKT":
                fill_sell(round_tick(bar_open - tick_size(bar_open), "BUY"), quantity, tag)
            elif kind == "BUY":
                if bar_open <= price:
                    fill_buy(bar_open, quantity)
                elif bar_low <= price:
                    fill_buy(price, quantity)
            elif kind == "SELL":
                if bar_open >= price:
                    fill_sell(bar_open, quantity, tag)
                elif bar_high >= price:
                    fill_sell(price, quantity, tag)
        orders = []
        closes.append(bar_close)
        if position > 0 and average > 0:
            mae = min(mae, (bar_low / average - 1.0) * 100.0)
            peak_high = max(peak_high, bar_high)
        if done:
            continue
        # 2) 존 게이트 — 전일 확정 SMA, 현재가는 봉 종가
        deviation = (bar_close - average_20) / average_20 * 100.0
        # 개장 이격 구제 — 09:03 판정에서 이격만 못 넘긴 날, 구제 시각의 현재가로 같은 식을 한 번 더 잰다.
        # 이 봉이 닫힌 시각(= hhmm + interval_min)이 구제 시각에 처음 닿는 봉에서 판정한다. 주문은 다음 봉 체결.
        if parameters.entry_rescue_hhmm > 0 and not rescue_done and not entry_allowed and atr_ok:
            bar_close_minute = hhmm // 100 * 60 + hhmm % 100 + parameters.interval_min
            if bar_close_minute >= parameters.entry_rescue_hhmm // 100 * 60 + parameters.entry_rescue_hhmm % 100:
                rescue_done = True
                if parameters.entry_open_deviation_min_percent <= deviation <= parameters.entry_open_deviation_max_percent:
                    entry_allowed = True
                    rescued = True
        up_threshold = parameters.entry_upper_percent + (parameters.zone_hysteresis_percent if in_zone else 0.0)
        if parameters.entry_lower_percent > 0.0:
            low_threshold = parameters.entry_lower_percent - (parameters.zone_hysteresis_percent if in_zone else 0.0)
        else:                                          # 눌림 슬리브: SMA20 아래 −pullback(유지는 −(pullback+hyst))
            low_threshold = -(parameters.pullback_percent + (parameters.zone_hysteresis_percent if in_zone else 0.0))
        band = low_threshold <= deviation <= up_threshold
        zone = al and band
        hold_zone = zone
        in_zone = zone
        last_bar = bar_index == bar_count - 1

        def liquidate(tag: str) -> None:
            nonlocal orders
            if position > 0:
                if last_bar:
                    fill_sell(bar_close, position, tag)
                else:
        ...
        elif parameters.stop_loss_percent > 0:
            stop_price = average * (1.0 - parameters.stop_loss_percent / 100.0)
        if stop_price > 0 and position > 0 and bar_close <= stop_price:
            liquidate("stop")
            cooldown_until = bar_index + parameters.stop_cooldown_bars
            continue
        if (parameters.trail_arm_percent > 0 and position > 0 and average > 0
                ...
                and bar_close <= peak_high * (1.0 - parameters.trail_percent / 100.0)):
            liquidate("trail_peak")
            cooldown_until = bar_index + parameters.stop_cooldown_bars
            continue
        if not zone:
            continue
        warming = len(closes) < parameters.simple_moving_average_period
        simple_moving_average = average_20 if warming else float(np.mean(closes[-parameters.simple_moving_average_period:]))
        if (parameters.trail_simple_moving_average_exit and not warming and position > 0
                and bar_close < simple_moving_average * (1.0 - parameters.trail_simple_moving_average_tolerance_percent / 100.0)):
            liquidate("trail")
            cooldown_until = bar_index + parameters.stop_cooldown_bars     # 엔진도 트레일 뒤 같은 쿨다운(L469)
            continue
        in_cooldown = bar_index < cooldown_until
        ...
        if (position <= 0 and not in_cooldown and confirmed and bar_index >= reentry_until and hhmm >= parameters.entry_from_hhmm
                and entry_allowed):
            buy_price = round_tick(bar_close if parameters.base_on_price else min(simple_moving_average, bar_close), "BUY")
            if buy_price >= bar_close:
                buy_price = round_tick(bar_close - tick_size(bar_close), "BUY")
        ...
        if position > 0:
            ...
            left = position
            sell_base = average if parameters.sell_base_average else max(simple_moving_average, bar_close)
            for split_step_index in range(1, parameters.split_step_count + 1):
                if left <= 0:
                    break
                sell_price = round_tick(sell_base * (1.0 + parameters.deviation_sell_percent * split_step_index / 100.0), "SELL")
                if sell_price <= bar_close:
                    sell_price = round_tick(bar_close, "SELL")
        ...
        if parameters.buy_split_steps > 0 and not warming and not in_cooldown:
            for split_step_index in range(1, parameters.buy_split_steps + 1):
                buy_price = round_tick(bar_close * (1.0 - parameters.deviation_buy_percent * split_step_index / 100.0), "BUY")
    ...
    return_percent = realized / max_deployed * 100.0 if max_deployed > 0 else 0.0
    return {"buys": buys, "sells": sells, "buy_notional": buy_notional, "sell_notional": sell_notional,
            "gross_pnl": gross, "net_pnl": realized, "max_deployed": max_deployed, "ret_pct": return_percent,
            "r": return_percent / R_DENOM_PCT, "mae_pct": mae, "exits": json.dumps(exits, ensure_ascii=False),
            "entry_allowed": bool(entry_allowed), "rescued": bool(rescued), "carry": carry_out}
...
def close_carry(carry: dict, cost: CostSpec) -> dict:
    ...
    return {"buys": 0, "sells": 1, "buy_notional": 0.0, "sell_notional": price * quantity,
            ...
            "exits": json.dumps({"carry_end": 1}, ensure_ascii=False),
            "entry_allowed": False, "rescued": False, "carry": None}
...
def load_pairs(path: Path) -> list[tuple[str, str]]:
    ...
    for line in text.splitlines():
        ...
        out += [(str(ticker).zfill(6), str(record["ymd"])) for ticker in record["tickers"]]
    return out
...
def _replay_pairs(pairs: list[tuple[str, str]], variants: dict[str, Params], since: str, until: str) -> tuple[list[dict], dict]:
    ...
    cost = LIVE
    book = DailyBook({ticker for ticker, _ in pairs})
    ...
    for ticker, day in sorted(pairs, key=lambda x: (x[0], x[1])):   # 종목별 날짜 순 — 보유를 다음 날로 넘기려면 필수
        ...
        for name, parameters in variants.items():
            ...
            result = replay_day(bars, moving_averages, parameters, cost, atr14, carries.pop((name, ticker), None))
            if result["carry"]:
                carries[(name, ticker)] = result["carry"]
            rows.append({"variant": name, "ticker": ticker, "ymd": day, "aligned_prev": aligned(moving_averages, parameters.align_tolerance_percent),
...
def main() -> int:
    ...
    parser.add_argument("--since", default="00000000")
    parser.add_argument("--until", default="99999999")
    parser.add_argument("--variant", default=None, help="하나만 돌릴 때")
    parser.add_argument("--set", dest="vset", default="base", choices=("base", "exec", "live", "entry", "carry", "carry_filter", "notional", "carry_wide", "carry_wide2", "carry_wide3", "carry_fine", "rescue"),
                    help="base=v1~v4, exec=16_trendx_execution 격자(ATR 스탑·진입 지연), live=09-21 눌림 슬리브 손절·익절·트레일 격자")
    parser.add_argument("--jobs", type=int, default=1, help="프로세스 수(종목 단위로 나눈다)")
    ...
    table = {"base": VARIANTS, "exec": EXEC_VARIANTS, "live": LIVE_VARIANTS, "entry": ENTRY_VARIANTS, "carry": CARRY_VARIANTS, "carry_filter": CARRY_FILTER_VARIANTS,
             ...
             "rescue": RESCUE_VARIANTS}[arguments.vset]
    variants = table if not arguments.variant else {arguments.variant: table[arguments.variant]}
    pairs = load_pairs(Path(arguments.pairs))
    results = run(pairs, variants, arguments.since.replace("-", ""), arguments.until.replace("-", ""), arguments.jobs)
    out = Path(arguments.out)
    results.to_csv(out.with_name(out.stem + "_days.tsv"), sep="\t", index=False, float_format="%.4f")
    monthly = monthly_table(results)
    monthly.to_csv(out, sep="\t", index=False, float_format="%.4f")
