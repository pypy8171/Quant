...
기대값 근거:
  ...
  - 나머지: `Quant/src/risk/OrderGate.cpp` 수식을 손으로 푼 값. 수수료 = 가격×수량×0.00015(1003행),
    거래세 = 매도 가격×수량×0.0018(1004행), 매수 평단 (1057-1058행), 매도 실현손익 (1096-1097행),
    부분 매도 평단 불변(1100행), 전량 매도 리셋(1104-1106행), 초과 매도 0 클램프(1082행),
    평단 미상 손익 0(1089-1093행), 매수 수수료 즉시 차감(1128행).
  - 케이스 11: 원장 CSV `Quant/build_win/logs/trades_20260910.csv` ORD-000033(ITB_441270) 실제 행.
...
from backtest.costs import LIVE, CostSpec, fill_result, tick_size  # noqa: E402
...
def test_golden_sequence_buy_partial_sell_add_full_sell():
    ...
    # 3. SELL 4 @1200 부분 매도 → (1200−1050)×4 − 4800×0.00015 − 4800×0.0018 = 600 − 0.72 − 8.64 = 590.64
    fill_3 = ledger.on_fill("005930", "SELL", 1200.0, 4, LIVE)
    assert_won(fill_3.realized_pnl, 590.64, "케이스3 실현손익")
    assert_won(fill_3.average_price, 1050.0, "케이스3 평단 유지")
    assert fill_3.net_quantity == 6
    assert_won(fill_3.fill.tax, 8.64, "케이스3 세금")
    assert_won(fill_3.fill.net, 4800.0 - 0.72 - 8.64, "케이스3 실수령")
    ...
    # 5. SELL 10 @1000 전량 → (1000−990)×10 − 1.5 − 18 = 80.5, 평단·수량 리셋
    fill_5 = ledger.on_fill("005930", "SELL", 1000.0, 10, LIVE)
    assert_won(fill_5.realized_pnl, 80.5, "케이스5 실현손익")
    assert fill_5.net_quantity == 0
    assert ledger.quantity("005930") == 0
    assert_won(ledger.average_price("005930"), 0.0, "케이스5 평단 리셋")

    # 누적 = −0.75 −0.825 +590.64 −0.54 +80.5 = 669.025 (매수 수수료 즉시 차감, 1128행)
    assert_won(ledger.realized_pnl, 669.025, "케이스1~5 누적 실현손익")


def test_golden_low_price_one_won_tick():
    """케이스 6: 저가주(호가 1원). BUY 1000 @1234 → 수수료 185.1. SELL 1000 @1235 → 1000 − 185.25 − 2223 = −1408.25"""
    ledger = PositionLedger()
    ...
    assert_won(sell.fill.tax, 2223.0, "케이스6 세금")
    assert_won(sell.realized_pnl, -1408.25, "케이스6 실현손익")


def test_golden_high_price_single_share():
    """케이스 7: 고가주 1주. BUY 1 @1,000,000 → 수수료 150. SELL 1 @1,010,000 → 10000 − 151.5 − 1818 = 8030.5"""
    ledger = PositionLedger()
    ...
    assert_won(sell.realized_pnl, 8030.5, "케이스7 실현손익")
    assert_won(ledger.realized_pnl, 8030.5 - 150.0, "케이스7 누적")


def test_golden_one_share_small_gain():
    """케이스 8: 1주 8000 → 8200. 200 − 8200×0.00015(1.23) − 8200×0.0018(14.76) = 184.01"""
    ledger = PositionLedger()
    ledger.on_fill("000003", "BUY", 8000.0, 1, LIVE)
    sell = ledger.on_fill("000003", "SELL", 8200.0, 1, LIVE)
    assert_won(sell.realized_pnl, 184.01, "케이스8 실현손익")
...
def test_golden_sell_without_basis():
    ...
    assert_won(sell.fill.tax, 27.0, "케이스9 세금")
...
def test_golden_oversell_clamps_to_zero():
    ...
    (10100−10000)×5 − 50500×0.00015(7.575) − 50500×0.0018(90.9) = 401.525"""
    ledger = PositionLedger()
    ...
    assert_won(sell.realized_pnl, 401.525, "케이스10 실현손익")


def test_golden_ledger_csv_row_2026_09_10():
    """케이스 11: 원장 실제 행. 평단 7864.88에서 1주 8200 매도 → 319.13, 1주 8190 매도 → 309.15 (CSV는 소수 2자리)."""
    ledger = PositionLedger()
    ledger.on_fill("441270", "BUY", 7864.88, 2, LIVE)
    sell_8200 = ledger.on_fill("441270", "SELL", 8200.0, 1, LIVE)
    sell_8190 = ledger.on_fill("441270", "SELL", 8190.0, 1, LIVE)
...
def test_fill_result_slippage_and_impact():
    ...
    specification = CostSpec(commission_percent=0.015, sell_tax_percent=0.18, slippage_ticks=1, impact_percent=0.05)
    ...
    assert_won(specification.roundtrip_percent, (0.00015 + 0.0005) * 100 + (0.00015 + 0.0018 + 0.0005) * 100, "왕복 %")
