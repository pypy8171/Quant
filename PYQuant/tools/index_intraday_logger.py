...
설계 원칙:
  * **원시 스냅샷만 적재.** price/change/change_rate/sign/ts만 남긴다. g/v/p/a·기준점·
    임계는 아직 검증 안 된 가정(회의 reviewer V5)이라 되돌릴 수 없는 로그에 굽지 않는다.
    파생지표는 price 시계열에서 오프라인으로 언제든 재계산 가능 → 규칙을 바꿔가며 제거실험.
  ...
  * **다중 프록시 동시 적재.** 0001(코스피)·1001(코스닥)·2001(KOSPI200) 3코드를 함께
    쌓는다(회의 data-sourcer: ITB=중소형/코스닥이면 0001과 거동 괴리 → 시장별 기준점 필요).
